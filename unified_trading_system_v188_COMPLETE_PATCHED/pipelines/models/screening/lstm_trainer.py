...
# Setup logging
# BASE_PATH should be the project root (4 levels up: screening -> models -> pipelines -> root)
...
class LSTMTrainer:
    ...
    def __init__(self, config_path: Optional[str] = None):
        ...
        if config_path is None:
            # Use same pattern as other screening modules: parent.parent gives us pipelines/models/
            config_path = Path(__file__).resolve().parent.parent / 'config' / 'screening_config.json'
        ...
        self.lstm_sequence_length = self.training_config.get('sequence_length', 60)  # LSTM input sequence length
        
        # Paths - LSTM models are saved in finbert_v4.4.4/models/saved_models/
        self.models_dir = BASE_PATH / 'finbert_v4.4.4' / 'models' / 'saved_models'
    ...
    def train_stock_model(
        ...
        try:
            ...
            # Debug: show BASE_PATH and which paths we're checking
            logger.info(f"BASE_PATH resolved to: {BASE_PATH}")
            logger.info(f"Checking local FinBERT: {finbert_path_relative}")
            logger.info(f"  -> exists: {finbert_path_relative.exists()}")
            logger.info(f"Checking AATelS FinBERT: {finbert_path_aatels}")
            logger.info(f"  -> exists: {finbert_path_aatels.exists()}")
            ...
            if finbert_path_relative.exists():
                ...
                raise FileNotFoundError(
                    ...
                    f"  1. {finbert_path_relative} (local) - exists: {finbert_path_relative.exists()}\n"
                    f"  2. {finbert_path_aatels} (AATelS) - exists: {finbert_path_aatels.exists()}"
                )
            ...
            if finbert_base not in sys.path:
                ...
                logger.info(f"[OK] Added finbert_base to sys.path[0]")
            else:
            ...
            logger.info(f"[OK] Import successful using importlib!")
            
            # Train the model using FinBERT's training function
            ...
            final_val_loss = train_results.get('final_val_loss', 0)
            
            # Log results
            logger.info(f"[OK] {symbol}: Training completed in {training_time:.1f}s")
            ...
            logger.error(f"[X] {symbol}: Training failed after {training_time:.1f}s")
    ...
    def train_batch(
        ...
        # FIX v1.3.15.169: Save model registry for dashboard use
        if trained_count > 0:
            logger.info("Saving model registry for dashboard...")
            self.save_model_registry(results)
        else:
            logger.warning("No models trained successfully, skipping registry save")
    ...
    def save_model_registry(self, training_results: List[Dict]) -> None:
        """
        Save registry of trained LSTM models for dashboard use.
        
        Args:
            training_results: List of training result dictionaries from train_batch()
        """
        try:
            registry = {}
            current_time = datetime.now(self.timezone)
            
            # Build registry from training results
            for result in training_results:
                ...
                model_path = self.models_dir / f'{symbol}_lstm_model.h5'
                scaler_path = self.models_dir / f'{symbol}_lstm_scaler.pkl'
                ...
                registry[symbol] = {
                    ...
                    'trained_date': current_time.strftime('%Y-%m-%d'),
                    'trained_time': current_time.strftime('%H:%M:%S'),
                    'file_modified': mod_time.strftime('%Y-%m-%d %H:%M:%S'),
                    'file_size_mb': round(file_size_mb, 2),
                    'samples': result.get('samples', 0),
                    'epochs': result.get('epochs', self.epochs),
                    'validation_accuracy': result.get('val_accuracy', 0.0),
                    ...
                    'data_range_start': result.get('data_start'),
                    'data_range_end': result.get('data_end')
                }
            
            if not registry:
                logger.warning("No successful models to save in registry")
                return
            ...
            logger.info(f"  Models: {', '.join(registry.keys())}")
            logger.info("="*80)
...
def main():
    ...
    if args.mode == 'stats':
        ...
        # Show helpful message if no models exist
        if stats['total_models'] == 0:
            print("\n" + "="*80)
            print("NO LSTM MODELS TRAINED YET")
            print("="*80)
            print("\nThis is normal if you haven't run training yet.")
            print("\nTo train your first models:")
            print("  1. Start small (recommended): RUN_LSTM_TRAINING_FIXED.bat --max-stocks 3")
            print("  2. Or train specific stocks: RUN_LSTM_TRAINING_FIXED.bat --symbols CBA.AX BHP.AX")
            print("\nTraining time: 5-15 minutes per stock")
            print("First 3 stocks will take approximately 15-45 minutes")
            print("="*80)
        ...
        if args.symbols:
            ...
            # Load from config
            config_path = BASE_PATH / 'pipelines' / 'models' / 'config' / 'asx_sectors.json'
            with open(config_path, 'r') as f:
        ...
        if args.symbols:
            ...
            if state_path.exists():
                ...
                # Pipeline state doesn't exist, fall back to top stocks from config
                logger.warning(f"Pipeline state not found at {state_path}")
                logger.info("Using top stocks from ASX sectors config instead")
                
                # Load top stocks from each sector
                config_path = BASE_PATH / 'pipelines' / 'models' / 'config' / 'asx_sectors.json'
                with open(config_path, 'r') as f:
                    sectors_config = json.load(f)
                
                # Get top stocks from each sector (first 3-5 from each)
                top_stocks = []
                max_per_sector = 3 if args.max_stocks and args.max_stocks <= 10 else 5
                
                print(f"\n[INFO] No pipeline state found, using top {max_per_sector} stocks per sector")
                print(f"[INFO] To train specific stocks, use: --symbols CBA.AX BHP.AX CSL.AX")
                print()
                
                for sector_name, sector_data in sectors_config['sectors'].items():
                    sector_stocks = sector_data['stocks'][:max_per_sector]
                    for symbol in sector_stocks:
                        top_stocks.append({'symbol': symbol, 'sector': sector_name})
                    logger.info(f"Selected {len(sector_stocks)} stocks from {sector_name}")
                
                # Limit to max_stocks if specified
                if args.max_stocks:
                    top_stocks = top_stocks[:args.max_stocks]
                
                training_queue = top_stocks
                
                print(f"\n[INFO] Will train {len(training_queue)} stocks")
                for i, stock in enumerate(training_queue[:10], 1):
                    print(f"  {i}. {stock['symbol']}")
                if len(training_queue) > 10:
                    print(f"  ... and {len(training_queue) - 10} more")
                print()
