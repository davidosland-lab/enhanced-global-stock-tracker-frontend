...
class FinBERTSentimentAnalyzer:
    ...
    v185: Class-level model caching to prevent reloading every cycle
    """
    ...
    _load_lock = None  # Will be initialized as threading.Lock()
    ...
    def __init__(self, model_name: str = "ProsusAI/finbert"):
        ...
        # Initialize lock for thread-safe loading
        if FinBERTSentimentAnalyzer._load_lock is None:
            import threading
            FinBERTSentimentAnalyzer._load_lock = threading.Lock()
        
        # Load model if available and not already loaded
        if FINBERT_AVAILABLE and not FinBERTSentimentAnalyzer._shared_model_loaded:
            with FinBERTSentimentAnalyzer._load_lock:
                # Double-check after acquiring lock
                if not FinBERTSentimentAnalyzer._shared_model_loaded:
                    logger.info("Loading FinBERT model for the first time (shared instance)...")
                    self._load_model()
                else:
        ...
        try:
            ...
            try:
                if local_cache:
                    ...
                    logger.info("[OK] FinBERT loaded from local cache (no download)")
                else:
                    ...
                    logger.info("[OK] FinBERT downloaded and cached for future use")
                ...
                self.tokenizer = AutoTokenizer.from_pretrained(self.model_name)
                self.model = AutoModelForSequenceClassification.from_pretrained(self.model_name)
                logger.info("[OK] FinBERT downloaded successfully and cached")
            ...
            # v185: Store in class-level cache for reuse
            FinBERTSentimentAnalyzer._shared_model = self.model
            FinBERTSentimentAnalyzer._shared_tokenizer = self.tokenizer
            FinBERTSentimentAnalyzer._shared_model_loaded = True
            
            logger.info("FinBERT model loaded successfully and ready for use (cached for future instances)")
            return True
