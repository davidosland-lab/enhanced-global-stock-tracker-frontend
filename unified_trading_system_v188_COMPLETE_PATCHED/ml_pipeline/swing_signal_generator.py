...
try:
    import os
    # Set KERAS_BACKEND if not already set
    ...
    error_msg = str(e)
    if "keras" in error_msg.lower():
        logging.warning(
            f"[WARN]  Keras not installed - LSTM predictions will use fallback method (70% accuracy)\n"
            f"    To enable LSTM neural networks (75-80% accuracy), run: INSTALL_KERAS_FINAL.bat\n"
            f"    Installation takes 2-5 minutes and requires ~2GB disk space\n"
            f"    Error details: {e}"
        )
    else:
...
class SwingSignalGenerator:
    ...
    def __init__(
        ...
        confidence_threshold: float = 0.48,  # v188: Lowered from 0.55 to enable 48-65% trades
        use_lstm: bool = True,
        ...
        if fast_mode:
            logger.info(f"   [FAST] FAST MODE: LSTM uses optimized fallback for speed")
    ...
    def generate_signal(
        ...
        try:
            ...
            # If LSTM not available, redistribute 25% weight to other components
            if not lstm_available:
                # Reweight: Sentiment 35%, Technical 35%, Momentum 20%, Volume 10%
                combined_score = (
                    sentiment_score * 0.35 +
                    lstm_score * 0.0 +  # LSTM unavailable, use fallback in lstm_score
                    technical_score * 0.35 +
                    momentum_score * 0.20 +
                    volume_score * 0.10
                )
                logger.debug(f"{symbol}: LSTM unavailable, using reweighted components (Sentiment 35%, Technical 35%, Momentum 20%)")
            else:
                # Standard weights
                combined_score = (
    ...
    def _analyze_lstm_v185(
        ...
        """
        v185: Enhanced LSTM analysis with availability flag
        
        Returns:
            Tuple of (lstm_score, is_lstm_available)
            - lstm_score: Score from -1.0 to +1.0
            - is_lstm_available: True if real LSTM used, False if fallback
        """
        if not self.use_lstm or not LSTM_AVAILABLE or self.fast_mode:
            # Fallback: use enhanced technical analysis
            try:
                prices = analysis_window['Close'].values
                # Multi-period moving average crossover
                if len(prices) >= 20:
                    short_ma = prices[-5:].mean()
                    med_ma = prices[-10:].mean()
                    long_ma = prices[-20:].mean()
                    
                    # Trend strength: compare all three
                    trend_score = 0.0
                    if short_ma > med_ma > long_ma:
                        trend_score = (short_ma / long_ma - 1) * 10
                    elif short_ma < med_ma < long_ma:
                        trend_score = (short_ma / long_ma - 1) * 10
                    else:
                        trend_score = (short_ma / long_ma - 1) * 5  # weaker signal
                    
                    return np.clip(trend_score, -1.0, 1.0), False
                else:
                    return 0.0, False
            except:
                return 0.0, False
        ...
        try:
            # Check data requirements for LSTM
            if len(full_data) < 60:
                logger.debug(f"{symbol}: Insufficient data for LSTM ({len(full_data)} days, need 60) - using fallback")
                prices = analysis_window['Close'].values
                if len(prices) >= 20:
                    short_ma = prices[-5:].mean()
                    long_ma = prices[-20:].mean()
                    return np.clip((short_ma / long_ma - 1) * 10, -1.0, 1.0), False
                return 0.0, False
            
            # FIX v1.3.15.169: Try to load pre-trained model first, then train as fallback
            if symbol not in self.lstm_models:
                # Try loading pre-trained model from overnight pipeline
                loaded = self._load_lstm_model(symbol)
                
                # If loading failed, train new model as fallback
                if not loaded:
                    logger.debug(f"{symbol}: No pre-trained LSTM model, training from scratch...")
                    self._train_lstm_model(symbol, full_data)
            
            # If model still not available, use enhanced fallback
            if symbol not in self.lstm_models:
                logger.debug(f"{symbol}: LSTM training failed, using enhanced fallback")
                prices = analysis_window['Close'].values
                if len(prices) >= 20:
                    short_ma = prices[-5:].mean()
                    med_ma = prices[-10:].mean()
            ...
            if len(scaled_prices) < self.lstm_sequence_length:
                logger.debug(f"{symbol}: Scaled prices insufficient ({len(scaled_prices)} < {self.lstm_sequence_length})")
                return 0.0, False
            ...
            return np.clip(lstm_score, -1.0, 1.0), True  # LSTM available and used
            
        except Exception as e:
            logger.debug(f"{symbol}: LSTM error: {e}, using fallback")
    ...
    def _analyze_lstm(
        ...
        try:
            ...
            if symbol not in self.lstm_models:
                ...
                # If loading failed, train new model as fallback
                if not loaded:
                    logger.info(f"[TRAIN] No pre-trained model for {symbol}, training from scratch...")
                    self._train_lstm_model(symbol, full_data)
    ...
    def _load_lstm_model(self, symbol: str) -> bool:
        ...
        FIX v1.3.15.169: Load pre-trained models instead of training each time
        ...
        try:
            ...
            # Try to find models directory (multiple possible locations)
            base_path = Path(__file__).resolve().parent.parent
            models_dir = base_path / 'finbert_v4.4.4' / 'models' / 'saved_models'
            
            if not models_dir.exists():
                logger.warning(f"[LOAD] Models directory not found: {models_dir}")
                return False
            
            # Check for model registry first
            ...
            if registry_path.exists():
                try:
                    ...
                    models_registry = registry_data.get('models', {})
                    
                    if symbol in models_registry:
                        model_info = models_registry[symbol]
                        ...
                        if model_path.exists() and scaler_path.exists():
                            ...
                            val_accuracy = model_info.get('validation_accuracy', 0.0)
                            
                            logger.info(f"[LOADED] [OK] LSTM model for {symbol} (trained {trained_date}, acc={val_accuracy:.2f})")
                            return True
                        else:
                            logger.warning(f"[LOAD] Model files missing for {symbol}: model={model_path.exists()}, scaler={scaler_path.exists()}")
                    else:
                        logger.debug(f"[LOAD] {symbol} not in registry (available: {', '.join(list(models_registry.keys())[:5])}...)")
                        
                except json.JSONDecodeError as e:
                    logger.error(f"[LOAD] Error reading registry: {e}")
            else:
                logger.debug(f"[LOAD] Registry not found at {registry_path}")
            
            # Fallback: check for model files directly (no registry)
            model_path = models_dir / f'{symbol}_lstm_model.h5'
            scaler_path = models_dir / f'{symbol}_lstm_scaler.pkl'
            ...
            if model_path.exists() and scaler_path.exists():
                ...
                logger.info(f"[LOADED] [OK] LSTM model for {symbol} (from file, no registry)")
                return True
            
            logger.debug(f"[LOAD] No pre-trained model found for {symbol}")
            return False
            
        except Exception as e:
            logger.error(f"[LOAD] Error loading LSTM model for {symbol}: {e}")
            return False
    
    def _train_lstm_model(self, symbol: str, price_data: pd.DataFrame):
        """Train LSTM model for a symbol (fallback when pre-trained not available)"""
        try:
