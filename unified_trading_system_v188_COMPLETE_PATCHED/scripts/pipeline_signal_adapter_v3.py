...
class EnhancedPipelineSignalAdapter:
    ...
    def combine_signals(
        ...
        if ml_signal and self.use_ml_signals:
            ...
            ml_prediction = ml_signal.get('prediction', 0)  # Already numeric from _generate_ml_signal
            
            # DEBUG: Check types
            if not isinstance(ml_signal, dict):
                logger.error(f"[BUG] ml_signal is not a dict! Type: {type(ml_signal)}, Value: {ml_signal}")
                raise TypeError(f"ml_signal must be dict, got {type(ml_signal)}")
            
            # Combine: ML (60%) + Sentiment (40%)
            # ML prediction range: -1 (SELL) to +1 (BUY)
            # Convert to 0-1 scale: (prediction + 1) / 2
            ml_score_normalized = (ml_prediction + 1) / 2
            
            combined_score = (
                self.ml_weight * ml_score_normalized +
...
def test_enhanced_adapter():
    ...
    for market in ['AU', 'US', 'UK']:
        ...
        for signal in signals:
            ...
            combined_score = float(signal.get('combined_score', 0))
            confidence = float(signal.get('confidence', 0))
            
            logger.info(f"  {signal['symbol']}: {signal['action']} @ {position_size:.1%}")
            logger.info(f"    Source: {signal['source']}")
            logger.info(f"    Combined Score: {combined_score:.2f}")
            logger.info(f"    Confidence: {confidence:.0%}")
            if signal.get('ml_prediction'):
                ml_prediction = float(signal.get('ml_prediction', 0))
