...
try:
    from pipelines.models.screening.uk_overnight_pipeline import UKOvernightPipeline as OvernightPipeline
...
try:
    from pipelines.models.screening.stock_scanner import StockScanner as OriginalScanner
    ...
    def run_full_overnight_pipeline(self, mode='full'):
        ...
        try:
            ...
            try:
                ...
                uk_report_path = project_root / 'reports' / 'screening' / 'uk_morning_report.json'
                
                spi_sentiment = {}
                if uk_report_path.exists():
                    with open(uk_report_path, 'r') as f:
                        uk_report = json.load(f)
                        spi_sentiment = uk_report.get('spi_sentiment', {})
                        logger.info(f"[HTML REPORT] Loaded SPI sentiment from {uk_report_path}")
                else:
                    ...
                    logger.warning(f"[HTML REPORT] UK report JSON not found, using fallback sentiment")
        ...
        if self.use_original_modules:
            ...
            if results:
                ...
                try:
                    ...
                    if top_opps:
                        ...
                        logger.info(f"[ANALYTICS] [OK] Stored {pred_stats['stored']} UK prediction signals")
                        logger.info(f"[ANALYTICS]   Data path: data/uk/trades/")
            ...
            try:
                ...
                try:
                    ...
                    logger.info("[ANALYTICS] UK data logging complete\n")
