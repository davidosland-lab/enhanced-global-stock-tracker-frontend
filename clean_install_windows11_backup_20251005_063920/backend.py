...
# Import multi-source live data service (if available)
try:
    from multi_source_data_service import multi_source_aggregator, LiveDataPoint, MarketData
    MULTI_SOURCE_AVAILABLE = True
except ImportError:
    MULTI_SOURCE_AVAILABLE = False
    multi_source_aggregator = None

# Import market holiday system (if available)
try:
    from market_holidays import MarketHolidayCalendar, get_all_market_holidays_summary
    MARKET_HOLIDAYS_AVAILABLE = True
except ImportError:
    MARKET_HOLIDAYS_AVAILABLE = False

# Configure logging (moved up before Historical Data Manager)
logging.basicConfig(level=logging.INFO)
logger = logging.getLogger(__name__)

# Import Historical Data Manager for fast local storage
try:
    from historical_data_manager import HistoricalDataManager
    HISTORICAL_DATA_MANAGER_AVAILABLE = True
    historical_data_manager = HistoricalDataManager("historical_data")
    logger.info("📦 Historical Data Manager initialized - Fast local data access enabled")
except ImportError as e:
    logger.warning(f"⚠️ Historical Data Manager not available: {e}")
    HISTORICAL_DATA_MANAGER_AVAILABLE = False
...
async def download_historical_data(request: Dict[str, Any]):
    """
    Download and store historical data for specified symbols
    """
    try:
        if not HISTORICAL_DATA_MANAGER_AVAILABLE:
            return {
                "success": False,
        ...
        intervals = request.get("intervals", ["1d", "1h", "30m"])
        
        if not symbols:
            raise HTTPException(status_code=400, detail="No symbols provided")
        
        # Download data
        results = await historical_data_manager.download_historical_data(
            symbols=symbols,
            period=period,
            intervals=intervals
        )
        ...
        # Get statistics
        stats = historical_data_manager.get_data_statistics()
        
        return {
            "success": True,
            "message": f"Downloaded data for {len(results)} symbols",
...
async def get_historical_data(
    ...
    try:
        ...
        df = historical_data_manager.get_historical_data(
            symbol=symbol,
            start_date=start_date,
            end_date=end_date,
            interval=interval
        ...
        if df.empty:
            # Try to download if not in local storage
            logger.info(f"Data not found locally for {symbol}, downloading...")
            await historical_data_manager.download_historical_data(
                symbols=[symbol],
                period="1y",
                intervals=[interval]
            )
...
@app.post("/api/historical/update/{symbol}")
async def update_symbol_data(symbol: str, force: bool = Query(False)):
    """
    ...
    try:
        ...
        updated = historical_data_manager.update_symbol_data(symbol, force=force)
        
        return {
            "success": updated,
            "message": f"{'Updated' if updated else 'Skipped (recent data exists)'} {symbol}"
        }
...
async def get_best_models(symbol: str, metric: str = Query("accuracy")):
    """
    ...
    try:
        ...
        best_models = historical_data_manager.get_best_models(symbol, metric=metric)
        
        return {
            "success": True,
            "models": best_models.to_dict('records') if not best_models.empty else []
        }
...
async def batch_download_symbols():
    """
    Download historical data for commonly used symbols
    """
    try:
        if not HISTORICAL_DATA_MANAGER_AVAILABLE:
            return {
        ...
        # Common symbols to download
        symbols = [
            "CBA.AX", "^AORD", "^GSPC", "^FTSE", "^DJI", "^IXIC",
            "AAPL", "MSFT", "GOOGL", "AMZN", "NVDA", "TSLA",
            "BHP.AX", "CSL.AX", "WBC.AX", "ANZ.AX", "NAB.AX"
        ]
...
if __name__ == "__main__":
    ...
    # HARDCODED PORT 8002 FOR WINDOWS 11 DEPLOYMENT
    port = 8002  # Changed from 8000 to 8002 for Windows 11 compatibility
    logger.info(f"Starting server on http://localhost:{port}")
    logger.info(f"🚀 Windows 11 Mode - Hardcoded to port {port}")
    uvicorn.run(
        app, 
