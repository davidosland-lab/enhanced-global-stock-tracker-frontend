#!/usr/bin/env python3
"""
Fixed Real Data Server - Corrected Alpha Vantage method name
"""
...
def fetch_data():
    ...
    if YF_AVAILABLE and not '.' in symbol:
        ...
        if symbol in aus_symbols:
            try:
                ...
                df = yf.download(test_symbol, period=period, progress=False)
    ...
    if ALPHA_VANTAGE_API_KEY:
        try:
            ...
            av_data = av.fetch_daily_data(symbol)  # Changed from fetch_daily_adjusted
            ...
            if av_data is not None and len(av_data) > 0:
                ...
                return jsonify({
                    ...
                    'volume': av_data['volume'].tolist() if 'volume' in av_data else [],
                    'is_real_data': True
                })
        except Exception as e:
...
def main():
    print("\n" + "="*60)
    print("🚀 FIXED REAL DATA SERVER")
    print("="*60)
    print("✅ Using yf.download() for better reliability")
    print("✅ Correct Alpha Vantage method names")
    print(f"✅ API Key: {ALPHA_VANTAGE_API_KEY[:8]}...")
    print("="*60)
    print("\n📊 Open: http://localhost:8000")
    print("📊 Try: SPY, QQQ, MSFT, GOOGL\n")
