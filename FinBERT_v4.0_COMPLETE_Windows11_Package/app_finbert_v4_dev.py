...
def get_stock_data(symbol):
    """Get stock data with v4.0 ML predictions (with cached daily predictions)"""
    try:
        interval = request.args.get('interval', '1d')
        period = request.args.get('period', '1m')
        use_cache = request.args.get('use_cache', 'true').lower() == 'true'
        
        logger.info(f"v4.0 Request for {symbol} - interval: {interval}, period: {period}, use_cache: {use_cache}")
        ...
        # Try to get CACHED daily prediction (consistent regardless of chart timeframe)
        ml_prediction = None
        prediction_source = 'live'
        
        if use_cache:
            try:
                # Initialize prediction system if needed
                if not prediction_manager:
                    initialize_prediction_system()
                
                # Get cached daily EOD prediction (always based on 1 year of daily data)
                if prediction_manager:
                    cached_pred = prediction_manager.get_daily_eod_prediction(symbol.upper())
                    if cached_pred:
                        # Convert cached prediction format to ml_prediction format
                        ml_prediction = {
                            'prediction': cached_pred.get('prediction', 'HOLD'),
                            'predicted_price': cached_pred.get('predicted_price', current_price),
                            'confidence': cached_pred.get('confidence', 0),
                            'predicted_change_percent': cached_pred.get('predicted_change_percent', 0),
                            'model_type': 'Cached Daily EOD',
                            'sentiment': cached_pred.get('sentiment'),
                            'is_cached': cached_pred.get('is_cached', True),
                            'is_locked': cached_pred.get('is_locked', False),
                            'prediction_id': cached_pred.get('prediction_id'),
                            'prediction_date': cached_pred.get('prediction_date'),
                            'target_date': cached_pred.get('target_date')
                        }
                        prediction_source = 'cached'
                        logger.info(f"✓ Using CACHED daily prediction for {symbol} (ID: {cached_pred.get('prediction_id')})")
            except Exception as cache_error:
                logger.warning(f"Could not get cached prediction: {cache_error}")
        
        # Fallback: Generate live prediction if no cache
        if ml_prediction is None:
            logger.info(f"Generating LIVE prediction for {symbol} (chart-based)")
            ml_prediction = ml_predictor.get_ensemble_prediction(chart_data, current_price, symbol)
            ml_prediction['is_cached'] = False
            ml_prediction['is_locked'] = False
            prediction_source = 'live'
        ...
        response = {
            ...
            'features': {
                ...
                'prediction_caching': prediction_manager is not None
            },
            'interval': interval,
            'period': period,
        ...
        logger.info(f"v4.0 Response: {ml_prediction['prediction']} ({ml_prediction['confidence']}%) - Source: {prediction_source}")
...
# ============================================================================
# PREDICTION CACHING API ENDPOINTS
# ============================================================================
...
if __name__ == '__main__':
    ...
    print("⚙️  Initializing prediction caching system...")
    try:
        initialize_prediction_system()
        if prediction_manager:
            print("✓ Prediction caching: ACTIVE (predictions will be consistent)")
        else:
            print("○ Prediction caching: Not available (will use live predictions)")
    except Exception as e:
