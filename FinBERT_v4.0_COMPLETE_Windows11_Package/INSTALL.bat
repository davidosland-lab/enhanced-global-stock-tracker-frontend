...
echo.
echo Installing async HTTP library for news scraping...
pip install aiohttp beautifulsoup4 lxml
if %errorlevel% neq 0 (
    echo WARNING: Web scraping dependencies had issues
    echo Real-time news sentiment may be limited
)

echo.
echo Installing scheduling and timezone libraries...
pip install APScheduler pytz
if %errorlevel% neq 0 (
    echo WARNING: APScheduler installation had issues
    echo Prediction validation scheduler may be limited
)
...
if %errorlevel% neq 0 (
    echo ⚠ TensorFlow not available - LSTM features limited
)

python -c "import aiohttp; print('✓ aiohttp version:', aiohttp.__version__)" 2>nul
if %errorlevel% neq 0 (
    echo ⚠ aiohttp not available - News scraping limited
)
