...
No test data, no simulated data, no synthetic data - only real market data
Uses Yahoo Finance primary + Alpha Vantage fallback
"""
...
class MLPredictor:
    """Machine Learning prediction model for stock prices - uses only real historical data"""
    ...
    def prepare_features(self, df):
        """Prepare technical features from real data for ML model"""
        features = pd.DataFrame(index=df.index)
        
        # Price features from real data
        features['returns'] = df['Close'].pct_change()
        features['volatility'] = features['returns'].rolling(20).std()
        
        # Moving averages from real data
        ...
        # Volume features from real data
        features['volume_ratio'] = df['Volume'] / df['Volume'].rolling(20).mean()
        
        # Price position from real data
    ...
    def train(self, df):
        """Train the ML model on real historical data only"""
        if len(df) < 50:
            return False
        ...
        # Prepare target from real data (next day return)
    ...
    def predict(self, df, days=5):
        """Predict future stock prices based on real data patterns"""
        ...
        # Get last real features
        last_features = features.iloc[-1:].values
        last_features_scaled = self.scaler.transform(last_features)
        
        # Predict future returns based on real data model
        predictions = []
        current_price = float(df['Close'].iloc[-1])
        
        for i in range(days):
            # Predict next return based on real data patterns
            pred_return = self.model.predict(last_features_scaled)[0]
            
            # Limit prediction to realistic range based on historical volatility
            historical_volatility = df['Close'].pct_change().std()
            max_change = min(0.05, historical_volatility * 3)  # Cap at 5% or 3 std devs
            pred_return = np.clip(pred_return, -max_change, max_change)
...
class StockDataFetcher:
    """Data fetcher for real market data only - no synthetic/test data"""
    ...
    def fetch_yahoo_data(self, symbol: str, period: str = '1mo') -> tuple:
        ...
        try:
            ...
            print(f"Fetching real data for {symbol} from Yahoo Finance, period: {period}")
            
            # Create ticker
            ticker = yf.Ticker(symbol)
            
            # Get real market data based on period
            if period == '1d':
                ...
                hist = ticker.history(period=period, interval='1d', prepost=False, actions=False)
            
            if not hist.empty:
                print(f"Yahoo Finance: Retrieved {len(hist)} real data points")
            ...
            # Try alternative download method for real data
            print(f"Trying yf.download for real data of {symbol}")
            ...
            if not df.empty:
                ...
                print(f"Yahoo download: Got {len(df)} real data points")
                current_price = float(df['Close'].iloc[-1])
    ...
    def fetch_alpha_vantage_data(self, symbol: str, period: str = '1mo') -> tuple:
        ...
        try:
            symbol = self._add_au_suffix(symbol).replace('.AX', '')
            print(f"Fetching real data for {symbol} from Alpha Vantage, period: {period}")
            ...
            if 'Error Message' in data:
                print(f"Alpha Vantage: Symbol not found or invalid")
                return None, None, None
            
            if 'Note' in data:
                print(f"Alpha Vantage: API call frequency limit reached")
                return None, None, None
            
            if time_key not in data:
                print(f"Alpha Vantage: No real data available")
                return None, None, None
            
            # Parse real time series data
    ...
    def fetch_data(self, symbol: str, period: str = '1mo') -> tuple:
        """Fetch real market data only - no synthetic data"""
        # Try Yahoo Finance for real data
        df, source, price = self.fetch_yahoo_data(symbol, period)
        if df is not None and not df.empty:
            return df, source, price
        
        # Try Alpha Vantage for real data
        print(f"Yahoo failed, trying Alpha Vantage for real data of {symbol}")
...
def get_stock_data(symbol):
    """API endpoint for real stock data only"""
    try:
        ...
        if df is None or df.empty:
            return jsonify({
                'error': f'No real market data available for {symbol}. Please check the symbol or try again later.',
                'suggestions': [
                    'Verify the stock symbol is correct',
                    'Try a different time period',
                    'Check your internet connection',
                    'API rate limits may apply - wait a moment and retry'
                ]
            }), 404
        
        # Train ML model on real data and get predictions
        ml_results = {}
        if len(df) >= 50:
        ...
        return jsonify({'error': str(e), 'data_type': 'error'}), 500
    ...
    try:
        ...
        if df is None or df.empty:
            return jsonify({
                ...
                'success': False,
                'message': 'Only real market data is displayed. Please verify the symbol and try again.'
            }), 404
        
        # Create subplots for real data visualization
        fig = make_subplots(
            rows=4, cols=1,
            row_heights=[0.5, 0.15, 0.15, 0.2],
            subplot_titles=['Price (Real Data)', 'Volume', 'RSI', 'MACD'],
        ...
        # Add moving averages from real data
        if len(df) >= 20:
        ...
        # Add ML predictions based on real data
        ml_info = {}
        if show_ml and len(df) >= 50:
            ...
            if predictions:
                ...
                fig.add_trace(go.Scatter(
                    ...
                    mode='lines+markers',
                    name='ML Prediction (Real Data)',
                    line=dict(color='purple', width=2, dash='dash'),
                    marker=dict(size=8, color='purple')
                ), row=1, col=1)
                
                # Add confidence band based on real data volatility
                confidence = training_results.get('test_score', 0.5) if training_results else 0.5
                historical_std = df['Close'].pct_change().std() * np.sqrt(5) * float(df['Close'].iloc[-1])
                std_dev = historical_std * (1 - confidence)
                upper_band = [p + std_dev for p in predictions]
                lower_band = [p - std_dev for p in predictions]
        ...
        # Update layout
        fig.update_layout(
            title=f'{symbol.upper()} - {period.upper()} - {source} (Real Data Only)',
            height=900,
            showlegend=True,
            hovermode='x unified',
        ...
        # Update y-axes with real data ranges
        fig.update_yaxes(
            title_text="Price ($)",
...
<html>
<head>
    <title>Stock Analysis System - Real Data Only</title>
...
<body>
    <div class="container">
        <div class="header">
            ...
            <p>Professional analysis using only real market data - No simulated or synthetic data</p>
        </div>
        
        <div class="info-banner">
            <strong>ℹ️ Real Market Data:</strong> This system displays only authentic market data from Yahoo Finance and Alpha Vantage. 
            If data is unavailable, you will be notified - no synthetic data will be shown.
        </div>
        ...
        <div class="controls">
            ...
            <div>
                <button onclick="loadChart()">🔄 Load Real Data</button>
                <button class="secondary" onclick="fetchData()">📊 Get Raw Data</button>
            </div>
            ...
            <div class="quick-symbols">
                ...
                <span class="symbol-btn" onclick="quickLoad('CBA')">CBA.AX</span>
                <span class="symbol-btn" onclick="quickLoad('BHP')">BHP.AX</span>
                <span class="symbol-btn" onclick="quickLoad('CSL')">CSL.AX</span>
            </div>
        </div>
        ...
        <div class="chart-container">
            <div id="chart">
                <div id="loading">Select options and click "Load Real Data" to begin</div>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        async function fetchData() {
            ...
            document.getElementById('chart').innerHTML = '<div id="loading">Fetching real market data...</div>';
            ...
            try {
                ...
                if (data.error) {
                    document.getElementById('chart').innerHTML = 
                        '<div class="error-message"><strong>No Real Data Available</strong><br>' + 
                        data.error + '<br><br>Suggestions:<ul>' +
                ...
                alert(`Real Data Retrieved Successfully!\\n` +
                      `Symbol: ${symbol}\\n` +
                      `Period: ${period}\\n` +
                      `Source: ${data.source}\\n` +
        ...
        async function loadChart() {
            ...
            document.getElementById('chart').innerHTML = 
                '<div id="loading">Loading real market data...</div>';
            ...
            try {
                ...
                if (!result.success) {
                    throw new Error(result.error || result.message || 'Failed to load real data');
                }
                ...
                document.getElementById('chart').innerHTML = 
                    '<div class="error-message"><strong>No Real Data Available</strong><br>' +
                    error.message + '<br><br>' +
                    'This system only displays real market data. Please:<ul>' +
                    '<li>Verify the stock symbol is correct</li>' +
                    '<li>Try a different time period</li>' +
                    '<li>Check your internet connection</li>' +
                    '<li>Wait a moment if API limits are reached</li>' +
                    '</ul></div>';
                document.getElementById('statsContainer').innerHTML = '';
            }
        }
        ...
        function updateStats(stats) {
                ...
                mlCard = `
                    <div class="stat-card">
                        ...
                        <div class="stat-change">
                            Confidence: ${(stats.ml_predictions.confidence * 100).toFixed(1)}%<br>
            ...
            document.getElementById('statsContainer').innerHTML = `
                <div class="stat-card">
                    <div class="stat-label">Current Price (Real)</div>
                ...
                <div class="stat-card">
                    ...
                    <div class="stat-change" style="color: #666;">
                        Real Data Points: ${stats.data_points}
                    </div>
                </div>
                <div class="stat-card">
                    ...
                    <div class="stat-change" style="color: #666;">
                        Symbol: ${stats.symbol}<br>
                        Type: ${stats.data_type || 'Real Market Data'}
                    </div>
                </div>
                ${mlCard}
            `;
...
if __name__ == '__main__':
    ...
    print("=" * 70)
    print("NO SYNTHETIC DATA - NO TEST DATA - NO SIMULATED DATA")
    print("=" * 70)
    print("Features:")
    print("✅ Real market data from Yahoo Finance")
    print("✅ Real market data from Alpha Vantage (fallback)")
    print("✅ ML predictions based on real historical patterns")
    print("✅ All technical indicators from real data")
    print("✅ No random/synthetic data generation")
    print("=" * 70)
