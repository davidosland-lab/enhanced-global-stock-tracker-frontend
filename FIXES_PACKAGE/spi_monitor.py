...
class SPIMonitor:
    ...
    def _get_asx_state(self) -> Dict:
        """
        Get current ASX 200 state with retry logic for rate limiting
        ...
        for attempt in range(max_retries):
            try:
                ...
                ticker = yf.Ticker(self.asx_symbol)
                hist = ticker.history(period='5d')
                
                if hist.empty:
                    logger.warning("No ASX data available")
                ...
                if len(hist) >= 5:
                    five_day_change = ((last_close - hist['Close'].iloc[0]) / hist['Close'].iloc[0]) * 100
                else:
                ...
                if '429' in error_msg or 'too many requests' in error_msg:
                    if attempt < max_retries - 1:
                        logger.warning(f"Rate limit hit for ASX data, retrying in {retry_delay * (attempt + 1)}s")
                        continue
                    else:
                        logger.warning(f"Rate limit exceeded for ASX data after {max_retries} attempts")
