...
class SPIMonitor:
    ...
    def __init__(self, config_path: str = None):
        """
        Initialize SPI Monitor (ALPHA VANTAGE MODE)
        ...
        # Initialize Alpha Vantage fetcher (Yahoo Finance disabled)
        self.data_fetcher = AlphaVantageDataFetcher(cache_ttl_minutes=240)
        
        logger.info("SPI Monitor initialized (Alpha Vantage mode)")
    ...
    def _get_asx_state(self) -> Dict:
        """
        Get current ASX 200 state using Alpha Vantage
        ...
        try:
            ...
            if hist is None or hist.empty:
                logger.warning("No ASX data available from Alpha Vantage")
                return {'available': False}
            
            # Get last 5 days of data
            if len(hist) < 2:
                logger.warning("Insufficient ASX data (need at least 2 days)")
                return {'available': False}
            ...
            logger.error(f"Error fetching ASX data from Alpha Vantage: {e}")
                ...
                try:
                    ...
                    if '429' in error_msg or 'too many requests' in error_msg:
                        if attempt < max_retries - 1:
                            logger.warning(f"Rate limit hit for {symbol}, retrying in {retry_delay * (attempt + 1)}s")
                            continue
                        else:
