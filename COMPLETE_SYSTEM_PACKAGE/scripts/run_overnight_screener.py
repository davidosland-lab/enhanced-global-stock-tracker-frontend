...
# Add parent directory to path (package root)
...
def setup_logging(log_dir: str = None):
    ...
    if log_dir is None:
        log_dir = Path(__file__).parent.parent / "logs" / "screening"
...
class OvernightScreener:
    ...
    def __init__(self, config_path: str = None):
        ...
        if config_path is None:
            config_path = Path(__file__).parent.parent / "models" / "config" / "screening_config.json"
    ...
    def _initialize_components(self):
        ...
        try:
            # Initialize scanner with BATCH FETCHING enabled (optimized for overnight scans)
            self.scanner = StockScanner(use_batch_fetching=True, cache_ttl_minutes=30)
            self.logger.info("  ✓ Stock Scanner initialized (batch fetching enabled)")
    ...
    def _scan_stocks(self, sectors: List[str] = None, test_mode: bool = False):
        """Scan stocks from all sectors using optimized batch fetching"""
        ...
        for sector_name in sectors:
            try:
                ...
                if test_mode:
                    # In test mode, manually limit symbols
                    sector_data = self.scanner.sectors[sector_name]
                    symbols = sector_data['stocks'][:5]
                    sector_weight = sector_data['weight']
                    
                    # Use batch scanning even in test mode
                    stocks = self.scanner._scan_sector_batch(symbols, sector_weight, top_n=30)
                else:
                    # Use scanner's scan_sector which automatically uses batch fetching
    ...
    def _save_results(self):
        ...
        try:
            ...
            results_dir = Path(__file__).parent.parent / "reports" / "screening_results"
