...
try:
    ...
    from .factor_view import FactorViewBuilder
    from .macro_beta import MacroBetaCalculator
except ImportError:
...
class OvernightPipeline:
    ...
    def run_full_pipeline(self, sectors: List[str] = None, stocks_per_sector: int = 30) -> Dict:
        ...
        try:
            ...
            # 🆕 Phase 2.2: Macro Beta Calculation
            try:
                logger.info("\n" + "="*80)
                logger.info("PHASE 2.2: MACRO BETA CALCULATION")
                logger.info("="*80)
                self.status['phase'] = 'macro_beta'
                self.status['progress'] = 30

                symbols = [s.get('symbol') for s in scanned_stocks if s.get('symbol')]
                beta_map = self.macro_beta_calc.compute_betas(symbols)
                for stock in scanned_stocks:
                    sym = stock.get('symbol')
                    stock_betas = beta_map.get(sym, {})
                    stock['macro_betas'] = stock_betas
                    stock['beta_xjo'] = stock_betas.get('xjo')
                    stock['beta_lithium'] = stock_betas.get('lithium')
            except Exception as e:
                logger.warning(f"Macro beta calculation failed: {e}")
    ...
    def _finalize_pipeline(self, scored_stocks: List[Dict], spi_sentiment: Dict, report_path: str) -> Dict:
        ...
        top_opps_detailed = [
            ...
            for opp in top_opportunities
              ...
              # Build factor view (per-stock factor contributions + aggregates)
        try:
            factor_view = self.factor_builder.build_factor_view(scored_stocks)
            factor_paths = self.factor_builder.save_factor_view(
                factor_view,
                date_override=report_date
            )
        except Exception as e:
            logger.warning(f"Factor view generation failed: {e}")
            factor_view = {}
            factor_paths = {}
        ...
        results = {
            ...
            'summary': summary,
            'factor_view': {
                'summary': factor_view.get('summary', {}) if 'factor_view' in locals() else {},
                'paths': factor_paths if 'factor_paths' in locals() else {}
            },
            'statistics': {
                'total_stocks_scanned': len(scored_stocks),
