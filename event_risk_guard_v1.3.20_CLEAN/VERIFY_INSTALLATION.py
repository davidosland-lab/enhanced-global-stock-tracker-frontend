...
def check_python_packages():
    ...
    for package, description in required_packages.items():
        try:
            ...
            # Handle both ImportError and ValueError (numpy version detection issue)
            if isinstance(e, ValueError) and 'numpy' in str(e).lower():
                print_success(f"{package:15s} - {description} (version detection issue, but installed)")
            else:
                print_error(f"{package:15s} - NOT INSTALLED ({description})")
                all_installed = False
        except Exception as e:
            print_warning(f"{package:15s} - WARNING: {str(e)[:50]}")
