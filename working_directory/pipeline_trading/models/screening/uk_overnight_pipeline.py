...
Main orchestration module for UK market screening workflow.
Coordinates all modules and handles error recovery, progress tracking, and logging.

Workflow:
1. Initialize all components
2. Fetch UK market sentiment (FTSE 100, VIX)
3. Scan all sectors (100 stocks)
...
# Import UK-specific modules
try:
    from .uk_stock_scanner import UKStockScanner
...
class UKOvernightPipeline:
    """
    Orchestrates the complete UK market overnight screening pipeline.
    """
    ...
    def __init__(self):
        ...
        logger.info("="*80)
        logger.info("UK OVERNIGHT STOCK SCREENING PIPELINE - STARTING")
        logger.info("="*80)
        logger.info(f"Start Time: {datetime.now(self.timezone).strftime('%Y-%m-%d %H:%M:%S %Z')}")
        
        try:
            self.scanner = UKStockScanner()
            self.market_monitor = UKMarketMonitor()
            ...
            logger.info("✓ All required UK market components initialized successfully")
        except Exception as e:
            logger.error(f"✗ Component initialization failed: {e}")
            raise
    
    def run_full_pipeline(self, sectors: List[str] = None, stocks_per_sector: int = 20) -> Dict:
        """
        Run the complete UK overnight screening pipeline
        
        Args:
            sectors: List of sector names to scan (None = all sectors)
            stocks_per_sector: Number of stocks to scan per sector (default 20 for UK)
    ...
    def _fetch_us_market_sentiment(self) -> Dict:
        """Fetch UK market sentiment (FTSE 100, VIX, etc.)"""
        ...
        try:
            ...
            logger.info(f"  FTSE 100: {sentiment['sp500']['price']:.2f} ({sentiment['sp500']['day_change']:+.2f}%)")
    ...
    def _analyze_market_regime(self) -> Dict:
        ...
        logger.info("Analyzing UK market regime (FTSE 100)...")
