"""
UK Stock Scanner - yahooquery ONLY Implementation
Specialized for UK market (LSE)

Key Differences from ASX Scanner:
- No .AX suffix on tickers
- UK market hours and timezone
- Higher volume/market cap thresholds
- UK-specific technical indicators
"""
...
class UKStockScanner:
    """
    UK Stock Scanner using ONLY yahooquery
    
    Features:
    - Stock validation (price, volume, market cap)
    - Technical analysis (RSI, MA, volatility)
    - Scoring system (0-100)
    - Sector-wise scanning for UK markets
    """
    
    def __init__(self, config_path: str = None):
        """Initialize US scanner with config"""
        if config_path is None:
            config_path = Path(__file__).parent.parent / "config" / "uk_sectors.json"
        ...
        # UK-specific selection criteria with defaults
        ...
        self.logger = logger
        logger.info(f"UK Stock Scanner initialized with {len(self.sectors)} sectors")
    ...
    def validate_stock(self, symbol: str) -> bool:
        ...
        for attempt in range(max_retries):
            try:
                ...
                # Price check (UK market: $5-$1000 typical range)
                ...
                # Volume check (UK market: typically higher volume)
