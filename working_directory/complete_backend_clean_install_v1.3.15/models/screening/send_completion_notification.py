...
class CompletionNotifier:
    ...
    def _build_summary_report(self, progress_data: Optional[Dict],
        ...
        lines.append("")
        lines.append("[OK] Pipeline completed successfully!")
        lines.append("")
        ...
        if progress_data:
            ...
            for stage_name in completed_stages:
                lines.append(f"  [OK] {stage_name.replace('_', ' ').title()}")
            lines.append("")
    ...
    def _send_email_notification(self, summary: str, timestamp: datetime,
        ...
        try:
            ...
            msg['Subject'] = f"[OK] FinBERT Screener Complete - {timestamp.strftime('%Y-%m-%d %H:%M')}"
            ...
            print(f"[OK] Completion notification email sent successfully")
            
        except Exception as e:
            logger.error(f"Failed to send completion notification email: {e}")
            print(f"[X] Failed to send email notification: {e}")
