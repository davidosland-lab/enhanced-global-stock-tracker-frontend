...
class StockScanner:
    ...
    def validate_stock(self, symbol: str, verbose: bool = False) -> bool:
        """
        Validate stock meets selection criteria with tiered volume thresholds
        
        Args:
            symbol: Stock ticker symbol
            verbose: If True, log detailed failure reasons
        ...
        for attempt in range(max_retries):
            try:
                ...
                if hist is None or hist.empty:
                    if verbose:
                        logger.info(f"  └─ No price data available")
                    return False
                ...
                # Price check (ASX: A$0.50 - A$500 typical range)
                min_price = self.criteria.get('min_price', 0.50)
                max_price = self.criteria.get('max_price', 500.0)
                if not (min_price <= current_price <= max_price):
                    if verbose:
                        logger.info(f"  └─ Price A${current_price:.2f} outside range A${min_price}-A${max_price}")
                    return False
                
                # Tiered volume check (ASX: generally lower volume than US)
                min_avg_volume = self.criteria.get('min_avg_volume', 100000)
                
                # Adjust thresholds based on stock price
                if current_price < 2.0:
                    # Small caps: reduce volume requirement by 70%
                    effective_volume_threshold = min_avg_volume * 0.3
                elif current_price < 10.0:
                    # Mid caps: reduce volume requirement by 50%
                    effective_volume_threshold = min_avg_volume * 0.5
                else:
                    # Large caps: full requirement
                    effective_volume_threshold = min_avg_volume
                ...
                if avg_volume < effective_volume_threshold:
                    if verbose:
                        logger.info(f"  └─ Volume {int(avg_volume):,} below threshold {int(effective_volume_threshold):,} "
                                  f"(stock price: A${current_price:.2f})")
                    return False
                ...
                if attempt < max_retries - 1:
                    ...
                    if verbose:
                        logger.info(f"  └─ Data fetch error: {str(e)[:50]}")
                    return False
    ...
    def scan_sector(self, sector_name: str, top_n: int = 10) -> List[Dict]:
        ...
        for i, symbol in enumerate(symbols):
            try:
                ...
                if not self.validate_stock(symbol, verbose=True):
                    logger.info(f"  [X] {symbol}: Failed validation")
                    continue
                ...
                if stock_data:
                    ...
                    logger.info(f"  [OK] {symbol}: Score {stock_data['score']:.0f}/100")
                else:
                    logger.info(f"  [X] {symbol}: Analysis failed")
                    
            except KeyboardInterrupt:
                logger.info("\n\nScan interrupted by user")
                break
            except Exception as e:
                logger.error(f"  [X] {symbol}: Error - {e}")
                continue
