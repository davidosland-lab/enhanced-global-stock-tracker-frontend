...
try:
    ...
    logger.info("[OK] All regime intelligence components initialized")
...
def login():
    ...
    if request.method == 'POST':
        ...
        if username in USERS and check_password_hash(USERS[username], password):
            ...
            logger.warning(f"[X] Failed login attempt: {username}")
...
def logout():
    ...
    session.clear()
    logger.info(f"[OK] User logged out: {username}")
...
def get_regime_data():
    ...
    try:
        ...
        logger.error(f"[X] Error fetching regime data: {e}", exc_info=True)
        return jsonify({
            'error': str(e),
            'timestamp': datetime.now().isoformat()
        }), 500
...
<body>
    <div class="container">
        <div class="header">
            <h1>[GLOBE] Market Regime Intelligence Dashboard</h1>
            <div class="user-info">
                <span>👤 {{ username }}</span>
    ...
    <script>
        ...
        async function loadRegimeData() {
            try {
                ...
                document.getElementById('loading').innerHTML = '[X] Error loading data: ' + error.message;
            }
        }
        ...
        function updateDashboard(data) {
            ...
            for (const [sector, impact] of Object.entries(data.sector_impacts)) {
                ...
                const label = impact < -0.5 ? '[X]' : impact < 0 ? '[!]' : impact < 0.3 ? '➖' : '[OK]';
...
if __name__ == '__main__':
    ...
    if not PRODUCTION:
        ...
        logger.warning("[!]  Default credentials: admin / change_me_in_production")
        logger.warning("[!]  Set PRODUCTION=true for production deployment")
