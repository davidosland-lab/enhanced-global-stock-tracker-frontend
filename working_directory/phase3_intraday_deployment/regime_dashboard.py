...
try:
    ...
    import traceback
    print(f"⚠️ Warning: Could not import regime modules")
    print(f"Error: {e}")
...
def initialize_components():
    ...
    if not MODULES_AVAILABLE:
        logger.error("❌ Regime modules not available")
        return False
    ...
    <script>
            ...
            try {
                ...
                if (data.error) {
                    document.getElementById('dashboard-content').innerHTML = `
                        <div class="card">
                            <div class="card-title">⚠️ Error</div>
                            <p>${data.error}</p>
                        </div>
                    `;
                    return;
                ...
                <div class="grid">
                    <div class="card">
                        ...
                        <div style="text-align: center; margin: 20px 0;">
                            <div class="regime-badge regime-primary">${regime.regime}</div>
                        </div>
                        <div class="confidence-bar">
                            <div class="confidence-fill" style="width: ${regime.confidence * 100}%">
                                Confidence: ${(regime.confidence * 100).toFixed(0)}%
                            </div>
                        </div>
                        <div class="metric">
                            <span class="metric-label">Strength</span>
                            <span class="metric-value">${regime.strength.toFixed(2)}</span>
                        </div>
        ...
        function renderMarketData(market) {
            ...
            return `
                <div class="grid">
                    <div class="card">
                        <div class="card-title">📈 US Markets</div>
                        ${renderMetric('S&P 500', market.sp500_return, '%')}
                        ${renderMetric('NASDAQ', market.nasdaq_return, '%')}
                    ...
                    <div class="card">
                        ...
                        ${renderMetric('AUD/USD', market.audusd_change, '%')}
        ...
        function renderCrossMarketFeatures(crossMarket) {
            ...
            return `
                <div class="grid">
                    <div class="card">
                        <div class="card-title">🌍 Cross-Market Features</div>
        ...
        function renderMetric(label, value, suffix) {
            ...
            const className = isNaN(numValue) ? 'neutral' : 
                             numValue > 0 ? 'positive' : 
                             numValue < 0 ? 'negative' : 'neutral';
