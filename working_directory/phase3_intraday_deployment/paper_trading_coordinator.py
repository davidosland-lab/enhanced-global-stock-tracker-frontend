...
class PaperTradingCoordinator:
        ...
        try:
            if YAHOOQUERY_AVAILABLE:
                ...
                if isinstance(hist, pd.DataFrame) and not hist.empty:
                    ...
                    logger.info(f"✓ Fetched {len(hist)} days of data for {symbol} (yahooquery)")
                    return hist
            ...
            if YFINANCE_AVAILABLE:
                ...
                if not hist.empty:
                    # Normalize columns
                    hist.columns = [col.capitalize() for col in hist.columns]
                    logger.info(f"✓ Fetched {len(hist)} days of data for {symbol} (yfinance)")
                    return hist
    ...
    def get_market_sentiment(self) -> float:
        ...
        Simulated by analyzing SPY and VIX
        ...
        try:
            ...
            logger.info(f"Market Sentiment: {sentiment:.1f}/100 (Daily: {daily_change:+.2f}%, 5-day: {five_day_change:+.2f}%)")
