...
class MarketDataFetcher:
    ...
    def fetch_market_data(self, use_cache: bool = True) -> Dict:
        ...
        logger.info("🌐 Fetching overnight market data...")
        
        try:
            if YAHOOQUERY_AVAILABLE:
                market_data = self._fetch_from_yahoo()
            else:
                logger.warning("⚠️ yahooquery not available - using mock data")
        ...
        try:
            ...
            for key, symbol in symbols.items():
                try:
                    ...
                    logger.warning(f"⚠️ Error fetching {key} ({symbol}): {e}")
                    market_data[f'{key}_change'] = 0.0
                    market_data[f'{key}_level'] = 0.0
            ...
            logger.error(f"❌ Yahoo Finance fetch error: {e}", exc_info=True)
            raise
    ...
    def _get_fallback_data(self) -> Dict:
        ...
        """
        logger.warning("⚠️ Using fallback neutral market data")
...
def test_market_data_fetcher():
    ...
    try:
        ...
        print("⚠️ Regime detector not available for testing")
