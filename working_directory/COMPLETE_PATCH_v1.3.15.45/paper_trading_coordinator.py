...
class PaperTradingCoordinator:
    ...
    def get_market_sentiment(self) -> float:
        ...
        **Enhanced in v1.3.15.45**: Now uses FinBERT v4.4.4 from morning report
        ...
        """
        # Try integrated sentiment first (FinBERT v4.4.4)
        if self.sentiment_analyzer:
            try:
                morning_data = self.sentiment_analyzer.load_morning_sentiment(market='au')
                
                if morning_data:
                    sentiment_score = morning_data['overall_sentiment']
                    recommendation = morning_data['recommendation']
                    finbert_data = morning_data.get('finbert_sentiment', {})
                    
                    # Store for decision making
                    self.last_market_sentiment = sentiment_score
                    self.sentiment_recommendation = recommendation
                    self.finbert_breakdown = finbert_data
                    
                    logger.info(f"[SENTIMENT] FinBERT v4.4.4: {sentiment_score:.1f}/100 ({recommendation})")
                    if finbert_data:
                        scores = finbert_data.get('overall_scores', {})
                        logger.info(f"            Breakdown: Neg {scores.get('negative', 0):.2%}, "
                                   f"Neu {scores.get('neutral', 0):.2%}, "
                                   f"Pos {scores.get('positive', 0):.2%}")
                    
                    return sentiment_score
                else:
                    logger.info("[SENTIMENT] Morning report not available, using SPY fallback")
                    
            except Exception as e:
                logger.warning(f"[SENTIMENT] Error loading morning report: {e}, using SPY fallback")
    ...
    def should_allow_trade(self, symbol: str, signal: Dict, sentiment_score: float) -> Tuple[bool, str]:
        ...
        **NEW in v1.3.15.45**: Respects negative sentiment from morning report
        ...
        if hasattr(self, 'sentiment_recommendation'):
            ...
            if recommendation in ['CAUTION', 'HOLD'] and sentiment_score < 45:
                ...
                if hasattr(self, 'finbert_breakdown'):
                    ...
                    # Warn but allow if moderate negative
                    elif negative_pct > 45:
                        reason = f"Moderate negative sentiment ({negative_pct:.1f}% negative), proceed with caution"
                        logger.warning(f"[CAUTION] {symbol}: {reason}")
                        return True, reason
        
        # Standard sentiment gates
        ...
        # All checks passed
        return True, "All sentiment gates passed"
        ...
        try:
            ...
            if not signal.get('type') == 'MANUAL':  # Don't block manual trades
                ...
                if gate == 'BLOCK':
                    ...
                    logger.warning(f"  → FinBERT Sentiment: {self.last_market_sentiment:.1f}/100")
                    return False
                    
                if gate == 'REDUCE':
                    logger.info(f"{symbol}: Position size reduced - {reason}")
                    # Position multiplier will be applied later
                    
                if gate == 'CAUTION':
                    logger.info(f"{symbol}: Trading with caution - {reason}")
