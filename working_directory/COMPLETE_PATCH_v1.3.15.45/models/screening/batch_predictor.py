...
class BatchPredictor:
    ...
    def __init__(self, config_path: str = None):
        ...
        if FINBERT_BRIDGE_AVAILABLE:
            try:
                ...
                logger.info("[OK] FinBERT Bridge initialized successfully")
        ...
        with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
            ...
            for future in as_completed(future_to_stock):
                ...
                try:
                    ...
                    completed += 1
                    # Log progress every 10 stocks or for first/last
                    if completed % 10 == 0 or completed == 1 or completed == total_stocks:
                        logger.info(f"  [{completed}/{total_stocks}] Processed {stock['symbol']} - Prediction: {prediction_data.get('prediction', 'N/A')} (Confidence: {prediction_data.get('confidence', 0):.1f}%)")
                    
                except Exception as e:
                    logger.error(f"  [X] Prediction failed for {stock['symbol']}: {e}")
                    # Add stock without prediction
                    stock['prediction_error'] = str(e)
                    results.append(stock)
                    completed += 1
        
        logger.info(f"[OK] Batch prediction complete: {len(results)}/{total_stocks} results")
        return results
        ...
        try:
            ...
            try:
                ...
                if isinstance(hist, pd.DataFrame) and not hist.empty:
                    ...
                    logger.debug(f"[OK] {symbol}: Data fetched from yahooquery ({len(hist)} days)")
            ...
            if (hist is None or hist.empty) and self.data_fetcher:
                try:
                    ...
                    if hist is not None and not hist.empty:
                        logger.debug(f"[OK] {symbol}: Data fetched from Alpha Vantage (backup)")
    ...
    def _calculate_ensemble_prediction(
        ...
        # Store full FinBERT sentiment scores (for overnight_pipeline aggregation)
        sentiment_scores = sentiment_pred.get('scores')
        if sentiment_scores:
            stock_data['sentiment_scores'] = sentiment_scores
            stock_data['sentiment_article_count'] = sentiment_pred.get('article_count', 0)
        ...
        return {
            ...
            'components': {
                ...
                'sentiment': {
                    ...
                    'scores': sentiment_pred.get('scores', {})  # Include FinBERT scores if available
                }
            },
        ...
        if self.finbert_bridge and self.finbert_components['lstm_available']:
            try:
                ...
                if lstm_result is not None and lstm_result.get('model_trained', False):
                    logger.debug(f"[OK] Using REAL FinBERT LSTM for {symbol}: direction={lstm_result['direction']:.3f}")
                    return {
        ...
        if self.finbert_bridge and self.finbert_components['sentiment_available'] and symbol:
            try:
                ...
                if sentiment_result is not None and sentiment_result.get('article_count', 0) > 0:
                    logger.debug(f"[OK] Using REAL FinBERT sentiment for {symbol}: {sentiment_result['sentiment']} ({sentiment_result['confidence']:.1f}%), {sentiment_result['article_count']} articles")
                    return {
                        'direction': sentiment_result['direction'],
                        'confidence': sentiment_result['confidence'] / 100.0,  # Convert to 0-1 scale
                        'scores': sentiment_result.get('scores', {}),  # Include full FinBERT scores
