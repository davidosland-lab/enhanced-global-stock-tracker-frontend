...
async def run_prediction_demo():
    ...
    # Test with configurable symbols from environment or defaults
    import os
    import sys
    ...
    # Use SymbolConfig if available, otherwise fall back to env var or default
    try:
        from symbol_config import SymbolConfig
        symbols = SymbolConfig.get_demo_symbols() if not symbols_arg else symbols_arg.split(',')
    except ImportError:
        # Fallback if symbol_config.py not available
        default_symbols = os.getenv('DEMO_SYMBOLS', 'AAPL,GOOGL,MSFT')
        symbols = symbols_arg.split(',') if symbols_arg else default_symbols.split(',')
