...
class AdaptiveMLIntegration:
    ...
    def __init__(self):
        ...
        logger.info(f"🤖 ML Integration initialized: {self.ml_source}")
        ...
        for path in possible_paths:
            if path.exists() and path.is_dir():
                ...
                if models_path.exists():
                    ...
                    logger.info(f"✅ Found local FinBERT models at: {path}")
                    sys.path.insert(0, str(path))
                    break
        
        if not self.finbert_available:
            logger.info("📦 Using archive ML pipeline (FinBERT models not found locally)")
    ...
    def _load_ml_modules(self):
        ...
        if self.finbert_available:
            ...
            try:
                ...
                logger.info("✅ Loaded local FinBERT + LSTM models")
            except Exception as e:
                logger.warning(f"⚠️ Failed to load local models: {e}")
                logger.info("📦 Falling back to archive ML pipeline")
                self.finbert_available = False
    ...
    def _load_archive_models(self):
        ...
        try:
            ...
            logger.info("✅ Loaded archive ML pipeline (LSTM, Transformer, GNN, Ensemble)")
        except Exception as e:
            logger.error(f"❌ Failed to load archive ML models: {e}")
            raise
    ...
    async def _generate_finbert_signal(
        ...
        try:
            ...
            logger.error(f"❌ FinBERT signal generation failed: {e}")
    ...
    async def _generate_archive_signal(
        ...
        logger.info(f"📊 Generating archive ML signal for {symbol}")
        ...
        try:
            ...
            logger.error(f"❌ Archive ML signal generation failed: {e}")
