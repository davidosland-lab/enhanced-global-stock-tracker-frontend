...
class LSTMTrainer:
    ...
    def train_stock_model(
        ...
        try:
            ...
            # Try multiple FinBERT locations (check for actual train_lstm.py file, not just directory)
            finbert_path_relative = BASE_PATH / 'finbert_v4.4.4'
            finbert_path_aatels = Path(r'C:\Users\david\AATelS\finbert_v4.4.4')
            
            # Priority: Use local FinBERT (relative path) first, then AATelS
            # Check if train_lstm.py actually exists, not just the directory
            if finbert_path_relative.exists() and (finbert_path_relative / 'models' / 'train_lstm.py').exists():
                finbert_path = finbert_path_relative
                logger.info(f"Using FinBERT from local: {finbert_path}")
                ...
                raise FileNotFoundError(
                    ...
                    f"  Neither location has the train_lstm.py module."
                )
