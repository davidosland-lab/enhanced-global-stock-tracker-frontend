...
"""
    ...
    def _train_lstm_models(self, scored_stocks: List[Dict]) -> Dict:
        ...
        Args:
            scored_stocks: List of scored stock opportunities
        ...
        try:
            logger.info("\n" + "="*80)
            logger.info("PHASE 4.5: LSTM MODEL TRAINING (Optional)")
            logger.info("="*80)
            
            max_models = lstm_config.get('max_models_per_night', 100)
            
            # Build training queue from top opportunities
            training_queue = []
            for stock in scored_stocks[:max_models]:
            ...
            logger.info(f"Training LSTM models for {len(training_queue)} stocks...")
            
            # Train models in batch
            training_results = self.lstm_trainer.train_batch(training_queue)
            
            trained_count = training_results.get('trained_count', 0)
            failed_count = training_results.get('failed_count', 0)
            total_time = training_results.get('total_time', 0)
            
            logger.info(f"[OK] LSTM Training Complete:")
            logger.info(f"     - Trained: {trained_count}/{len(training_queue)} models")
            logger.info(f"     - Failed: {failed_count}")
            logger.info(f"     - Time: {total_time:.2f} minutes")
