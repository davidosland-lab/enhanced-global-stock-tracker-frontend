...
class EmailNotifier:
    ...
    def send_notification(
        ...
        try:
            ...
            logger.info(f"[OK] Email notification sent: {notification_type} - {subject}")
            return True
        
        except Exception as e:
            logger.error(f"[X] Failed to send email notification: {str(e)}")
            return False
    ...
    def send_error(
        ...
        report_date = datetime.now(self.timezone).strftime('%Y-%m-%d')
        subject = f"[X] PIPELINE ERROR - {report_date}"
    ...
    def _generate_email_content(
        ...
        if notification_type == 'success':
            subject = f"[OK] Pipeline Completed - {report_date}"
    ...
    def _create_error_body(
        ...
        lines = []
        lines.append("[X] OVERNIGHT PIPELINE ERROR")
        lines.append("=" * 80)
        lines.append("")
...
def main():
    ...
    if args.type == 'test':
        ...
        if success:
            ...
            print("[X] Failed to send test email")
        ...
        if state_path.exists():
            ...
            if success:
                print("[OK] Morning report email sent successfully!")
            else:
                print("[X] Failed to send morning report email")
        ...
        if state_path.exists():
            ...
            if success:
                ...
                print("[X] Failed to send alert email (or no high-confidence opportunities)")
        ...
        if success:
            print("[OK] Error email sent successfully!")
        else:
            print("[X] Failed to send error email")
