...
class AlphaVantageDataFetcher:
    ...
    def _track_api_call(self):
        ...
        if self.api_calls_today > 450:  # Warning at 90% of limit
            logger.warning(f"[!]  High API usage: {self.api_calls_today}/500 calls today")
    ...
    def fetch_daily_data(self, ticker: str, outputsize: str = "compact") -> Optional[pd.DataFrame]:
        ...
        try:
            ...
            logger.info(f"[OK] Fetched {ticker}: {len(df)} days of data")
            return df
        ...
        for ticker in tickers:
            try:
                ...
                if hasattr(info, 'last_price') and info.last_price is not None and info.last_price > 0:
                    valid.append(ticker)
                    logger.debug(f"  [OK] {ticker}: Valid (yfinance)")
                    ...
                    logger.debug(f"  [X] {ticker}: No price data (yfinance)")
                    self._validation_cache[ticker] = {
                        'valid': False,
                        'timestamp': time.time()
                    }
                ...
                logger.debug(f"  [X] {ticker}: yfinance error - {str(e)[:50]}")
                self._validation_cache[ticker] = {
                    'valid': False,
                    'timestamp': time.time()
                }
            ...
            if ticker in self._validation_cache:
                ...
                if age_seconds < 3600:
                    if self._validation_cache[ticker]['valid']:
                        valid.append(ticker)
                        logger.debug(f"  [OK] {ticker}: Valid (cached)")
                    else:
                        logger.debug(f"  [X] {ticker}: Invalid (cached)")
                    continue
            ...
            try:
                ...
                if 'Global Quote' in data and data['Global Quote']:
                    ...
                    if '05. price' in quote and float(quote['05. price']) > 0:
                        is_valid = True
                        valid.append(ticker)
                        logger.debug(f"  [OK] {ticker}: Valid")
                    else:
                        logger.debug(f"  [X] {ticker}: No price data")
                else:
                    logger.debug(f"  [X] {ticker}: Not found")
                ...
                logger.debug(f"  [X] {ticker}: Error - {str(e)[:50]}")
    ...
    def fetch_batch(self, tickers: List[str], period: Optional[str] = "compact",
        ...
        logger.info(f"[!]  This will take ~{len(tickers) * self.rate_limit_delay / 60:.1f} minutes (rate limiting)")
        ...
        for i, ticker in enumerate(tickers):
            ...
            if df is not None and not df.empty:
                results[ticker] = df
                logger.info(f"  [{i+1}/{len(tickers)}] [OK] {ticker}: {len(df)} days")
            else:
                logger.warning(f"  [{i+1}/{len(tickers)}] [X] {ticker}: No data")
            ...
            for ticker in valid:
                ...
                try:
                    ...
                    if min_price is not None and last_close < min_price:
                        logger.debug(f"  [X] {ticker}: Price ${last_close:.2f} < ${min_price}")
                        continue
                    if max_price is not None and last_close > max_price:
                        logger.debug(f"  [X] {ticker}: Price ${last_close:.2f} > ${max_price}")
                        continue
                    if min_avg_volume is not None and avg_vol < min_avg_volume:
                        logger.debug(f"  [X] {ticker}: Volume {avg_vol:,.0f} < {min_avg_volume:,.0f}")
                        continue
                    
                    filtered.append(ticker)
                    logger.debug(f"  [OK] {ticker}: Passed filters")
