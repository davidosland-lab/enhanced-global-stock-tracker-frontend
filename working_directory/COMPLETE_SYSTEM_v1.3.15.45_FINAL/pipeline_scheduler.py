...
class PipelineScheduler:
    ...
    def _run_pipeline(self, market: MarketSchedule) -> bool:
        ...
        try:
            ...
            if result.returncode == 0:
                logger.info(f"[OK] {market.name} pipeline completed successfully")
                ...
                logger.error(f"[X] {market.name} pipeline failed (exit code {result.returncode})")
                if result.stderr:
            ...
            logger.error(f"[X] {market.name} pipeline timed out (>10 minutes)")
            return False
        except Exception as e:
            logger.error(f"[X] {market.name} pipeline error: {e}")
            import traceback
            traceback.print_exc()
            return False
    ...
    def test_schedule(self):
        ...
        for code, market in self.markets.items():
            ...
            logger.info(f"  Script Exists: {'[OK]' if market.script_path.exists() else '[X]'}")
