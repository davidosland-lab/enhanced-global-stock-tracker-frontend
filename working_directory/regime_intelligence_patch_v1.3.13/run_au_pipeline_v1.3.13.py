...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('logs/au_pipeline.log'),
...
class AUPipelineRunner:
    ...
    def __init__(self, symbols=None, capital=100000, config_path='config/live_trading_config.json', 
        ...
        logger.info("=" * 80)
        logger.info("AU MARKET PIPELINE RUNNER v1.3.13 - REGIME INTELLIGENCE EDITION")
        logger.info("=" * 80)
    ...
    def check_market_status(self):
        ...
        try:
            ...
            if status == MarketStatus.OPEN:
                logger.info("✅ ASX is OPEN - Trading enabled")
                return True
            elif status == MarketStatus.CLOSED:
                logger.warning("❌ ASX is CLOSED - No trading")
                # Get next open time
                ...
                logger.warning(f"⚠️ ASX Status: {status.name}")
                return False
    ...
    def validate_symbols(self):
        ...
        for symbol in self.symbols:
            if not symbol.endswith('.AX'):
                logger.warning(f"⚠️ Symbol {symbol} doesn't have .AX suffix - adding it")
                symbol = f"{symbol}.AX"
    ...
    def run(self, check_market_hours=True):
        ...
        try:
            ...
            logger.info("✅ AU Pipeline stopped successfully")
            return True
        except Exception as e:
