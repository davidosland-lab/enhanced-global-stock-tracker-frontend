...
class RegimeAwareOpportunityScorer:
    ...
    def __init__(self, config_path: Optional[str] = None):
        ...
        # Regime influence weight (0-1)
        # 0 = ignore regime, 1 = fully regime-driven
        # Default 0.4 = 40% regime influence, 60% stock fundamentals
        self.regime_weight = self.config.get('regime_weight', 0.4)
        
        logger.info("✅ Regime-Aware Opportunity Scorer initialized")
        logger.info(f"  Scoring Weights: {self.weights}")
        logger.info(f"  Regime Influence: {self.regime_weight*100:.0f}%")
    ...
    def score_opportunities(
        ...
        for stock in stocks_with_features:
            try:
                ...
                logger.error(f"❌ Scoring error for {stock.get('symbol', 'UNKNOWN')}: {e}", exc_info=True)
        ...
        if scored_stocks:
            ...
            logger.info(f"✅ Scoring complete. Top opportunity: {top_stock.get('symbol')} "
    ...
    def _calculate_opportunity_score(
        ...
        # regime_weight controls how much regime matters (default 0.4 = 40%)
        final_score = base_score + (scaled_regime_adjustment * self.regime_weight)
        ...
        if regime_signal < -0.3:
            impact_label = "❌ STRONG HEADWINDS"
            impact_color = "red"
        elif regime_signal < -0.1:
            impact_label = "⚠️ MODERATE HEADWINDS"
            impact_color = "orange"
        elif regime_signal > 0.3:
            impact_label = "✅ STRONG TAILWINDS"
            impact_color = "green"
        elif regime_signal > 0.1:
