...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('logs/uk_pipeline.log'),
...
class UKPipelineRunner:
    ...
    def check_market_status(self):
        ...
        try:
            ...
            if status == MarketStatus.OPEN:
                logger.info("✅ LSE is OPEN - Trading enabled")
                return True
            elif status == MarketStatus.CLOSED:
                logger.warning("❌ LSE is CLOSED - No trading")
                # Get next open time
                ...
                logger.warning(f"⚠️ LSE Status: {status.name}")
                return False
    ...
    def validate_symbols(self):
        ...
        for symbol in self.symbols:
            if not symbol.endswith('.L'):
                logger.warning(f"⚠️ Symbol {symbol} doesn't have .L suffix - adding it")
                symbol = f"{symbol}.L"
    ...
    def run(self, check_market_hours=True):
        ...
        try:
            ...
            logger.info("✅ UK Pipeline stopped successfully")
            return True
        except Exception as e:
