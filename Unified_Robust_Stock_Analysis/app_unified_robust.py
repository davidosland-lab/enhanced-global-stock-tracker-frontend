...
print("=" * 80)
print("UNIFIED ROBUST STOCK ANALYSIS SYSTEM")
print("=" * 80)
...
print("✓ Market Sentiment Indicators (VIX, Market Breadth, Yields, Dollar)")
print("✓ ML Predictions with RandomForest")
print("✓ Technical Analysis (RSI, MACD, Bollinger, SMA, EMA)")
print("✓ Chart Zoom Functionality")
print("✓ Australian Stock Support (.AX/.AUS)")
print("=" * 80)
...
class MarketSentimentAnalyzer:
    ...
    def get_treasury_yield(self):
        ...
        try:
            df, _, source = self.fetcher.fetch_yahoo('^TNX', period='5d')
            if df is not None and not df.empty:
                ...
                if current_yield < 3:
                    sentiment = "Low Yields (Risk-On)"
                    score = 0.5
                elif current_yield < 4:
                    sentiment = "Normal Yields"
                    score = 0
                elif current_yield < 5:
                    sentiment = "Rising Yields (Caution)"
                    score = -0.3
                else:
                    sentiment = "High Yields (Risk-Off)"
                    score = -0.6
    ...
    def get_dollar_index(self):
        """Get US Dollar Index"""
        try:
            df, _, source = self.fetcher.fetch_yahoo('DX-Y.NYB', period='5d')
            if df is not None and not df.empty:
                current = df['Close'].iloc[-1]
                prev = df['Close'].iloc[-2] if len(df) > 1 else current
                ...
                if change > 1:
                    sentiment = "Strong Dollar (Risk-Off)"
                    score = -0.5
                elif change > 0:
    ...
    def get_sector_rotation(self):
        ...
        try:
            for symbol, name in sectors.items():
                df, _, source = self.fetcher.fetch_yahoo(symbol, period='5d')
                if df is not None and not df.empty:
                    current = df['Close'].iloc[-1]
            ...
            if sector_performance:
                ...
                # Sentiment based on leading sectors
    ...
    def get_overall_sentiment(self):
        ...
        # Collect all sentiment indicators
        vix = self.get_vix()
        if vix:
        ...
        if treasury:
            sentiment_data['treasury_yield'] = treasury
            scores.append(treasury['score'])
        
        dollar = self.get_dollar_index()
        if dollar:
            sentiment_data['dollar_index'] = dollar
            scores.append(dollar['score'] * 0.5)  # Less weight
...
class MLPredictor:
    ...
    def prepare_features(self, df, technical_indicators):
        ...
        try:
            ...
            if 'MACD' in technical_indicators:
                features['macd'] = technical_indicators['MACD']
            if 'MACD_Signal' in technical_indicators:
                features['macd_signal'] = technical_indicators['MACD_Signal']
    ...
    def train(self, df, technical_indicators):
        """Train the ML model"""
        ...
        try:
            # Prepare features
            features = self.prepare_features(df, technical_indicators)
            ...
            y = df['Close'].shift(-1).pct_change().loc[features.index]
    ...
    def predict(self, df, technical_indicators, days=5):
        """Make predictions"""
        if not self.available:
            return None
        ...
        if self.model is None:
            ...
            self.train(df, technical_indicators)
        ...
        try:
            ...
            # Multi-day predictions (simplified)
            for d in [3, 5, 7]:
                # Simple extrapolation
                multi_return = next_return * np.sqrt(d)
                multi_price = current_price * (1 + multi_return)
                
                predictions.append({
                    'days': d,
                    'price': multi_price,
                    'return': multi_return * 100,
                    'confidence': max(40, 70 - d * 5)
                })
...
def get_predictions(symbol):
    ...
    try:
        # Fetch data
        df, info, source = data_fetcher.fetch(symbol, period='6mo')
        ...
        predictions = ml_predictor.predict(df, indicators)
...
def get_sector_rotation():
    ...
    try:
        sectors = sentiment_analyzer.get_sector_rotation()
...
<head>
    ...
    <title>Unified Robust Stock Analysis</title>
...
<body>
    <div class="container">
        ...
        <div class="header">
            ...
            <p style="color: #718096;">Yahoo Finance • Alpha Vantage • ML Predictions • Market Sentiment</p>
        ...
        <div class="tabs">
            ...
            <button class="tab" onclick="switchTab('indicators')">Technical Indicators</button>
            <button class="tab" onclick="switchTab('sentiment')">Market Sentiment</button>
            <button class="tab" onclick="switchTab('predictions')">ML Predictions</button>
        </div>
    ...
    <script>
        ...
        async function fetchPredictions() {
            ...
            showStatus('loading', 'Generating ML predictions...');
        ...
        function displayPredictions(data) {
            ...
            // Feature importance
            if (data.predictions.feature_importance) {
                html += '<h4 style="margin-top: 20px;">Top Features</h4>';
                html += '<div style="display: grid; gap: 5px;">';
                
                for (const feature of data.predictions.feature_importance) {
        ...
        function displaySentiment(data) {
            ...
            // Treasury Yield
            if (data.treasury_yield) {
                const treasury = data.treasury_yield;
                html += createSentimentCard('10Y Treasury', treasury.value, treasury.change, treasury.sentiment);
            }
            
            // Dollar Index
            if (data.dollar_index) {
                const dollar = data.dollar_index;
                html += createSentimentCard('Dollar Index', dollar.value, dollar.change, dollar.sentiment);
            }
...
if __name__ == '__main__':
    print("\nStarting Unified Robust Stock Analysis System...")
