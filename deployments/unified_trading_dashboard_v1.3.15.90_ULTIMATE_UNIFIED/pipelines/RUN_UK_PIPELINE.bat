@echo off
REM Run UK Market Overnight Pipeline
...
REM Output: reports/screening/uk_morning_report.json

chcp 65001 >nul
setlocal EnableDelayedExpansion

set "VENV=%~dp0..\finbert_v4.4.4\venv\Scripts\python.exe"
set "SCRIPT=%~dp0run_uk_pipeline.py"

echo ================================================================================
echo UK MARKET OVERNIGHT PIPELINE v1.3.15.87
echo ================================================================================
echo.
...
echo Features: FTSE 100 + VFTSE + GBP/USD
echo.
echo ================================================================================
echo.
...
if not exist "..\logs\screening\uk\errors" mkdir "..\logs\screening\uk\errors"
if not exist "..\reports\screening" mkdir "..\reports\screening"
if not exist "..\reports\csv_exports" mkdir "..\reports\csv_exports"
if not exist "..\data\uk" mkdir "..\data\uk"
echo.

REM Run pipeline
echo Starting UK market pipeline...
echo.
...
if %EXIT_CODE% EQU 0 (
    ...
    echo.
    echo Output Reports:
    echo   - JSON: ..\reports\screening\uk_morning_report.json
    echo   - CSV:  ..\reports\csv_exports\uk_screening_results_*.csv
    echo   - Logs: ..\logs\screening\uk\uk_overnight_pipeline.log
) else (
    echo PIPELINE FAILED - Exit Code: %EXIT_CODE%
    echo.
    echo Check logs for details:
    echo   - ..\logs\screening\uk\uk_overnight_pipeline.log
    echo   - ..\logs\screening\uk\errors\
)
echo ================================================================================
echo.
