...
# Add parent directory to path to access core/ and ml_pipeline/
sys.path.insert(0, str(Path(__file__).parent.parent))

# Configure logging with UTF-8 encoding for Windows BEFORE imports
import io
...
# Import regime intelligence modules BEFORE finbert_v4.4.4 path is added
# (finbert_v4.4.4 also has a models/ directory which would shadow the root models/)
try:
    ...
    logger.info("[OK] Regime intelligence modules imported successfully")
except ImportError as e:
    REGIME_INTELLIGENCE_AVAILABLE = False
    MarketDataFetcher = None
    MarketRegimeDetector = None
    RegimeAwareOpportunityScorer = None
    logger.warning(f"[!] Regime intelligence modules not available: {e}")

# Import sector scanner from finbert_v4.4.4
try:
    ...
    if finbert_models_path.exists():
        sys.path.insert(0, str(finbert_models_path))  # Add finbert/models/ directly
        from screening.stock_scanner import StockScanner  # Import from screening subdirectory
        logger.info("Successfully imported StockScanner from finbert_v4.4.4")
    else:
        logger.warning(f"FinBERT models path not found: {finbert_models_path}")
        StockScanner = None
except ImportError as e:
...
class AUPipelineRunner:
    ...
    def __init__(self, symbols=None, capital=100000, config_path='config/live_trading_config.json', 
                 use_sector_scan=False, sectors_config='../finbert_v4.4.4/models/config/asx_sectors.json',
    ...
    def check_market_status(self):
        ...
        try:
            ...
            logger.info(f"ASX Market Status: {status_info.status.value}")
            logger.info(f"Current Time (AEST): {now_aest.strftime('%Y-%m-%d %H:%M:%S %Z')}")
            
            if status_info.status == MarketStatus.OPEN:
    ...
    def run(self, check_market_hours=True):
        ...
        try:
            ...
            # Add parent directory to path to access pipelines module
...
Examples:
  ...
  python run_au_pipeline.py --list-presets
    ...
    parser.add_argument('--sectors-config', type=str, default='../finbert_v4.4.4/models/config/asx_sectors.json',
