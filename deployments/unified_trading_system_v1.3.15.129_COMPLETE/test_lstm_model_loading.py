...
def test_model_loading():
    ...
    if success:
        print("[PASS] ✓ AAPL model loaded successfully")
    else:
        print("[EXPECTED] ✓ AAPL model not loaded (files don't exist, expected behavior)")
    ...
    if not success:
        print("[PASS] ✓ MSFT model not loaded (not in registry, expected)")
    else:
...
def test_registry_structure():
    ...
    if registry_path.exists():
        ...
        if 'models' in registry:
            print(f"  ✓ Models section present ({len(registry['models'])} models)")
...
def test_analyze_lstm_flow():
    ...
    try:
        ...
        print(f"[PASS] ✓ Got LSTM score: {score:.3f}")
        
        # Check if model was cached
        if 'TSLA' in generator.lstm_models:
            print(f"[PASS] ✓ Model cached for future use")
        else:
...
def test_format_error_fixes():
    ...
    try:
        ...
        print(f"[PASS] ✓ Converted string to float: {conf:.2f}")
    except Exception as e:
    ...
    try:
        ...
        if isinstance(conf, (list, np.ndarray)):
            ...
            conf = float(conf)
        print(f"[PASS] ✓ Converted list to float: {conf:.2f}")
    except Exception as e:
        print(f"[FAIL] ✗ Error: {e}")
    ...
    try:
        ...
        if isinstance(conf, (list, np.ndarray)):
            ...
            conf = float(conf)
        print(f"[PASS] ✓ Converted array to float: {conf:.2f}")
    except Exception as e:
        print(f"[FAIL] ✗ Error: {e}")
...
def main():
    ...
    try:
        ...
        print("#"*80)
        print("\nSummary:")
        print("  ✓ Model loading logic implemented")
        print("  ✓ Registry structure validated")
        print("  ✓ _analyze_lstm flow working")
        print("  ✓ Format error fixes validated")
        print("\nNext steps:")
