...
class PaperTradingCoordinator:
    ...
    def __init__(
        ...
        # Try to restore pipeline state from disk (v1.3.15.182)
            ...
            try:
                ...
                if not hist.empty and len(hist) >= 20:
                    ...
                    # Reduce position on WAIT_FOR_DIP
                        ...
                        # Apply 50% position reduction for poor timing
                        sentiment_multiplier = self._get_sentiment_multiplier(sentiment_score)
                        return True, sentiment_multiplier * 0.5, reason
    ...
    def get_status_dict(self) -> Dict:
        ...
        return {
            ...
            'pipeline': {
                'reports_cached': len(self._overnight_reports_cache),
                'last_check': self._reports_last_check.isoformat() if self._reports_last_check else None,
                'processed_recommendations': list(self._processed_recommendations),
                'reports': self._overnight_reports_cache  # Save full pipeline reports
            },
