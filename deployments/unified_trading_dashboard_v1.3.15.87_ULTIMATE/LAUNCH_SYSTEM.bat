...
REM  Version: v1.3.15.87 ULTIMATE
REM  Date: 2026-02-03
REM  
REM  NEW in v1.3.15.87 ULTIMATE:
...
echo   Smart Launcher - v1.3.15.87 ULTIMATE (Two-Stage Intelligence System)
echo ═══════════════════════════════════════════════════════════════════════════
echo.
...
python scripts\run_au_pipeline_v1.3.13.py --full-scan --capital 100000 --ignore-market-hours
...
python scripts\run_us_full_pipeline.py --full-scan --capital 100000 --ignore-market-hours
...
python scripts\run_uk_full_pipeline.py --full-scan --capital 100000 --ignore-market-hours
...
start "Paper Trading Platform" python core\paper_trading_coordinator.py --config config\live_trading_config.json
...
if exist "core\unified_trading_dashboard.py" (
    ...
    echo %COLOR_RED%[X] core\unified_trading_dashboard.py not found%COLOR_RESET%
    echo.
    pause
)
...
if exist "core\dashboard.py" (
    ...
    echo %COLOR_RED%[X] core\dashboard.py not found%COLOR_RESET%
    echo.
    pause
)
...
echo.
echo v1.3.15.87 ULTIMATE - Two-Stage Intelligence System (75-85%% Win Rate)
echo Complete Package: Overnight Pipelines + Real-Time ML + FinBERT v4.4.4
echo.
timeout /t 2 >nul
exit /b 0
