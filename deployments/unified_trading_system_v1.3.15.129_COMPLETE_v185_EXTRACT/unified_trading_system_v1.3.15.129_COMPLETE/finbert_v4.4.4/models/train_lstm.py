...
def fetch_training_data(symbol: str, period: str = '2y') -> pd.DataFrame:
    ...
    try:
        ...
        logger.info(f"[OK] Successfully fetched {len(df)} days of data for {symbol}")
        return df
...
def train_model_for_symbol(symbol: str, epochs: int = 50, sequence_length: int = 60):
    ...
    logger.info(f"[OK] Data validation passed: {len(df)} data points available")
    ...
    logger.info(f"[OK] Features prepared: {len(available_features)} features")
    logger.info(f"  Features: {available_features}")
    
    try:
        # KERAS 3 FIX: Pass symbol for symbol-specific model paths
        predictor = StockLSTMPredictor(
            sequence_length=sequence_length,
            features=available_features,
            symbol=symbol  # Each stock gets its own model file
        )
    except Exception as e:
    ...
    try:
        ...
        logger.info(f"[OK] Test prediction: {test_prediction}")
    except Exception as e:
    ...
    logger.info(f"="*60)
    logger.info(f"[OK] LSTM training complete for {symbol}")
    logger.info(f"="*60)
