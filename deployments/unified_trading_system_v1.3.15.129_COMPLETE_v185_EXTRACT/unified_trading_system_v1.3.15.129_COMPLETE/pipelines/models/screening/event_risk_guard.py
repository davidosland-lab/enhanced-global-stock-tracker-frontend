...
class EventRiskGuard:
    ...
    def __init__(self, extra_providers: Optional[List] = None, csv_path: Optional[str] = None, market: str = 'AU'):
        """
        ...
        Args:
            ...
            market: Market identifier ('AU', 'UK', 'US') for regime engine configuration
        """
        self.market = market
        self.providers = [
        ...
        try:
            ...
            from .market_regime_engine import MarketRegimeConfig
            ...
            logger.info(f"[OK] Market Regime Engine initialized successfully ({market} market)")
    ...
    def refresh_market_data(self):
        """
        [OK] NEW: Force refresh of overnight market data for regime detection.
    ...
    def assess(self, ticker: str) -> GuardResult:
        ...
        if avg_sent is not None and avg_sent < NEG_SENTIMENT_THRES:
            risk += 0.25
            if not warning:
                warning = f"[!] Negative sentiment ({avg_sent:+.2f}) detected in recent news"
    ...
    def assess_batch(self, tickers: List[str]) -> Dict:
        ...
        """
        # [OK] NEW: Refresh market data before regime analysis
