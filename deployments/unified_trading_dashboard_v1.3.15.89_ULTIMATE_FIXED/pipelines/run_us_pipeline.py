"""
US Market Overnight Pipeline Runner
...
7. Report Generation (JSON + CSV + Email)
...
def main():
    """Main entry point for US market overnight pipeline"""
    parser = argparse.ArgumentParser(
        description='US Market Overnight Screening Pipeline'
    )
    parser.add_argument(
        '--sectors',
        nargs='+',
    ...
    parser.add_argument(
        ...
        choices=['full', 'test'],
        default='full',
        help='Execution mode: full (all sectors) or test (Technology only, 5 stocks)'
    )
    parser.add_argument(
        '--full-scan',
        action='store_true',
    ...
    try:
        ...
        if args.mode == 'test' or (not args.full_scan and args.sectors is None):
            logger.info("Running in TEST mode (Technology sector, 5 stocks)")
        ...
        print(f"Sentiment Score: {results['sentiment']['overall']['score']:.1f}/100")
