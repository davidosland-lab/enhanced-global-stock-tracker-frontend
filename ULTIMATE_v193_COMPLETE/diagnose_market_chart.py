...
def diagnose_market_chart():
    ...
    try:
        ...
        print(f"[OK] Fetched {len(hist)} data points")
    ...
    if len(market_hours_data) > 0:
        print("[OK] Data is available for ^FTSE")
        print(f"[OK] {len(market_hours_data)} data points during market hours")
        print(f"[OK] Percentage changes calculated successfully")
        print()
