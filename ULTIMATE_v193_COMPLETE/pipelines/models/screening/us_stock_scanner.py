...
class USStockScanner:
    ...
    def validate_stock(self, symbol: str, verbose: bool = False) -> bool:
        """
        Validate US stock meets selection criteria with tiered volume thresholds
        ...
        for attempt in range(max_retries):
            try:
                ...
                min_avg_volume = self.criteria.get('min_avg_volume', 500000)
                
                # Adjust thresholds based on stock price (same logic as UK)
    ...
    def scan_sector(self, sector_name: str, max_stocks: int = 30) -> List[Dict]:
        ...
        for i, symbol in enumerate(stocks, 1):
            try:
                ...
                # Validate stock with verbose output
                ...
                if analysis:
                    ...
                    logger.info(f"    [OK] {symbol}: Score {analysis['score']:.1f}/100")
                
                # Rate limiting
                time.sleep(0.1)
                
            except Exception as e:
                logger.info(f"  [{i}/{len(stocks)}] {symbol}: Error - {e}")
                continue
        
        # Sort by score (highest first)
        results.sort(key=lambda x: x['score'], reverse=True)
        
        logger.info(f"[OK] {sector_name}: {len(results)}/{len(stocks)} stocks analyzed")
        return results
