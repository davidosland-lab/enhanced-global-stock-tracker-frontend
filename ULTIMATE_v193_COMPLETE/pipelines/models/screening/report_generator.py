...
class ReportGenerator:
    ...
    def __init__(self, config_path: str = None):
        ...
        # FIX v1.3.15.115: Set base_path for report directory resolution
        # Go up from pipelines/models/screening/ to project root
        ...
        if Path(report_path_str).is_absolute():
            self.report_dir = Path(report_path_str)
        else:
            # Resolve relative to base_path (project root)
            self.report_dir = self.base_path / report_path_str
    ...
    def generate_morning_report(
        ...
        market_data: Dict = None  # FIX v1.3.15.171: Add market_data parameter
    ) -> str:
        """
        ...
        html_content = self._build_html_report(
            ...
            market_data=market_data  # FIX v1.3.15.171: Pass market data
        )
    ...
    def _build_html_report(
        ...
        regime_html = self._build_market_regime_section(event_risk_data, market_data)  # FIX v1.3.15.171: Pass market_data
...
"""
    ...
    def _build_market_overview(self, spi_sentiment: Dict) -> str:
        ...
        # v193: World Event Risk card (if available)
        world_risk_card = ""
        if spi_sentiment.get('world_event_risk'):
            world_risk = spi_sentiment['world_event_risk']
            risk_score = world_risk.get('world_risk_score', 50)
            risk_level = world_risk.get('risk_level', 'MODERATE')
            ...
            # Top topics
            topics = world_risk.get('top_topics', [])
            topics_text = ", ".join(topics[:3]) if topics else "N/A"
            
            world_risk_card = f"""
            <div class="metric-card">
                <div class="metric-label">World Event Risk</div>
                <div class="metric-value {risk_class}">{risk_emoji} {risk_level}</div>
                <div class="metric-change">{risk_score:.0f}/100 Risk Score</div>
                <div style="font-size: 0.9em; color: #6b7280; margin-top: 4px;">Topics: {topics_text}</div>
            </div>
            """
        
        return f"""
    <div class="section">
        <h2><span class="emoji">[UP]</span> Market Overview</h2>
...
"""
        ...
        return f"""
    <div class="section">
        <h2><span class="emoji">[!]</span> Caution Stocks (Sell Signals)</h2>
        {items}
    </div>
"""
...
"""
    ...
    def _build_market_regime_section(self, event_risk_data: Dict = None, market_data: Dict = None) -> str:
        """
        Build market regime analysis section
        FIX v1.3.15.171: Use market_data for regime detection if event_risk_data unavailable
        """
        # Try to get regime from event_risk_data first
        if event_risk_data and 'market_regime' in event_risk_data:
            regime = event_risk_data.get('market_regime', {})
        elif market_data:
            # FIX v1.3.15.171: Calculate regime from market_data
            regime = self._calculate_regime_from_market_data(market_data)
        else:
            return ""
...
"""
    ...
    def _calculate_regime_from_market_data(self, market_data: Dict) -> Dict:
        """
        Calculate basic market regime from overnight market data
        FIX v1.3.15.171: Fallback regime calculation when event_risk_data unavailable
        
        Args:
            market_data: Dictionary with overnight market data (S&P500, VIX, etc.)
        ...
        # Simple regime classification based on VIX and market moves
        # VIX thresholds: <15=low, 15-25=medium, >25=high volatility
        if vix_level < 15:
        ...
        # Adjust crash risk based on market direction
        us_avg_move = (sp500_change + nasdaq_change) / 2
        if us_avg_move < -2.0:  # Significant negative move
            crash_risk_score += 0.15
        elif us_avg_move < -1.0:
            crash_risk_score += 0.08
        
        # Cap crash risk
        crash_risk_score = min(crash_risk_score, 0.95)
        
        # Build regime probabilities based on current state
        if regime_label == 'low_vol':
            regime_probs = {0: 0.70, 1: 0.25, 2: 0.05}
        elif regime_label == 'medium_vol':
            regime_probs = {0: 0.20, 1: 0.60, 2: 0.20}
        else:  # high_vol
            regime_probs = {0: 0.05, 1: 0.25, 2: 0.70}
        
        return {
            'regime_label': regime_label,
            'crash_risk_score': crash_risk_score,
            'vol_1d': vol_annual / 252**0.5,  # Approximate daily vol
            'vol_annual': vol_annual,
            'regime_probabilities': regime_probs,
            'data_window': {
                'start': market_data.get('timestamp', 'N/A'),
                'end': market_data.get('timestamp', 'N/A')
            },
            'source': 'market_data_fallback',  # Mark as fallback calculation
            'vix_level': vix_level
        }
...
"""
    ...
    def save_trading_report(
        ...
        # Save to expected path for trading platform
...
def test_report_generator():
    ...
    print(f"[OK] File size: {Path(report_path).stat().st_size / 1024:.1f} KB")
