...
get_real_sentiment_for_symbol = None
...
def _load_finbert_if_needed():
    ...
    global get_sentiment_sync, get_real_sentiment_for_symbol
    ...
    try:
        ...
        from models.news_sentiment_real import get_sentiment_sync as gss, get_real_sentiment_for_symbol as grsfs
        finbert_analyzer = fa
        get_sentiment_sync = gss
        get_real_sentiment_for_symbol = grsfs
...
class EnhancedMLPredictor:
    ...
    def get_sentiment_for_symbol(self, symbol: str) -> Optional[Dict]:
        ...
        """
        # Try to load FinBERT if not already loaded
        if not self.finbert_enabled:
            _load_finbert_if_needed()
            self.finbert_enabled = FINBERT_AVAILABLE
        
        if not self.finbert_enabled or not get_sentiment_sync:
        ...
        try:
            ...
            logger.info(f"[OK] REAL Sentiment for {symbol}: {sentiment.get('sentiment').upper()} "
...
def train_model(symbol):
    """Train LSTM model for a specific symbol (supports dots like BHP.AX)"""
    ...
    try:
        ...
        if request.is_json:
            ...
            try:
                ...
                data = json_lib.loads(request.data.decode('utf-8')) if request.data else {}
            except:
                data = {}
        
        # Convert string numbers to integers (in case from form data)
        try:
            epochs = int(data.get('epochs', 50))
            sequence_length = int(data.get('sequence_length', 60))
        except (ValueError, TypeError):
            logger.warning(f"Invalid parameter types, using defaults")
            epochs = 50
            sequence_length = 60
        ...
        if 'error' in result:
            ...
            logger.error(f"Training failed for {symbol}: {error_msg}")
            return jsonify({
                'status': 'error',
                'message': error_msg,
                'symbol': symbol,
        ...
        logger.info(f"Reloading models after training {symbol}")
...
def initialize_prediction_system():
    ...
    if prediction_manager is None:
        try:
            ...
            logger.info("[OK] Prediction system initialized with multi-timezone scheduler")
...
def run_backtest():
    ...
    try:
        ...
        try:
            ...
            logger.info(f"[OK] Backtest results saved to: {results_file}")
...
def optimize_backtest_parameters():
    ...
    try:
        ...
        try:
            ...
            logger.info(f"[OK] Optimization results saved to: {results_file}")
            response['results_file'] = results_file
...
def initialize_trading_system():
    ...
    try:
        ...
        logger.info("[OK] Paper trading system initialized")
        return True
    except ImportError as e:
...
@app.route('/api/trading/positions/<path:symbol>/close', methods=['POST'])
...
if __name__ == '__main__':
    # CRITICAL: Load FinBERT on startup (was missing!)
    logger.info("Initializing FinBERT sentiment analysis...")
    _load_finbert_if_needed()
    
    # Refresh predictor's FinBERT status after loading
    ml_predictor.finbert_enabled = FINBERT_AVAILABLE
