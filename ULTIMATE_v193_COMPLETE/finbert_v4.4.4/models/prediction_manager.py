...
class PredictionManager:
    ...
    def get_daily_eod_prediction(self, symbol: str, force_refresh: bool = False) -> Dict:
        ...
        if not force_refresh:
            ...
            if cached:
                if self.market_tz:
                    ...
                    logger.info(f"[OK] Using cached prediction for {symbol} "
        ...
        if self.market_tz and not force_refresh:
            ...
            if not can_generate and "Market is OPEN" in reason:
                logger.warning(f"[WARN]  {symbol}: Cannot generate prediction - {reason}")
    ...
    def _fetch_chart_data(self, symbol: str, interval: str = '1d', 
        ...
        try:
            ...
            logger.info(f"[OK] Fetched {len(chart_data)} candles for {symbol} ({period}, {interval})")
            return chart_data
    ...
    def validate_predictions(self) -> Dict:
        ...
        logger.info(f"[OK] Validation complete: {validated_count} predictions validated, "
