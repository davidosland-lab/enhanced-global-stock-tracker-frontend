...
# FIX: Use importlib to avoid "No module named 'models.finbert_sentiment'" error
...
try:
    ...
    logger.info("[OK] FinBERT analyzer imported successfully (relative import)")
    ...
    try:
        ...
        if finbert_sentiment_path.exists():
            ...
            if spec and spec.loader:
                ...
                logger.info("[OK] FinBERT analyzer imported (importlib)")
            else:
...
def init_cache_db():
    ...
    try:
        ...
        conn.commit()
        conn.close()
        logger.info("[OK] News sentiment cache database initialized")
...
def fetch_yfinance_news(symbol: str) -> List[Dict]:
    ...
    try:
        ...
        logger.info(f"[OK] Fetched {len(articles)} articles for {symbol} using yfinance API")
...
def scrape_rba_pages(symbol: str) -> List[Dict]:
    ...
    try:
        ...
        logger.info(f"  [OK] RBA Media Releases: {len([a for a in articles if 'RBA:' in a['title']])} articles")
            ...
            for link in unique_links[:3]:  # Get 3 most recent speeches
                try:
                    ...
                    if title and len(title) > 10:
                        ...
                        logger.info(f"    [OK] Found speech: {title[:50]}...")
                
                except Exception as e:
                    logger.warning(f"    Error parsing RBA speech: {e}")
                    continue
        
        logger.info(f"  [OK] RBA Speeches: {len([a for a in articles if 'Speech' in a['title']])} articles")
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if chart_title:
                ...
                logger.info(f"    [OK] Found Chart Pack update")
    
    except Exception as e:
        logger.warning(f"  Failed to scrape RBA Chart Pack: {e}")
    
    logger.info(f"[OK] Scraped {len(articles)} articles from RBA official sources")
    return articles
...
def enrich_australian_news_context(articles: List[Dict], symbol: str) -> List[Dict]:
    ...
    for article in articles:
        ...
        if contexts_found:
            ...
            logger.info(f"  [OK] Tagged article with contexts: {context_labels}")
        
        enriched_articles.append(article)
    
    australian_count = sum(1 for a in enriched_articles if a.get('is_australian_news'))
    logger.info(f"[OK] Found {australian_count}/{len(articles)} articles with Australian market context")
...
def get_real_sentiment_for_symbol(symbol: str, use_cache: bool = True) -> Dict:
    ...
    try:
        ...
        if len(analyzed_articles) > 0:
            ...
            logger.info(f"[OK] REAL sentiment analysis complete for {symbol}: {aggregate_sentiment['sentiment'].upper()} ({aggregate_sentiment['confidence']:.1f}%)")
