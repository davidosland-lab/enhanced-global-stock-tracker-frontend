...
class PredictionDatabase:
    ...
    def init_prediction_tables(self):
        ...
        conn.commit()
        conn.close()
        logger.info("[OK] Prediction database tables initialized successfully")
    ...
    def get_prediction(self, symbol: str, date: str, timeframe: str = 'DAILY_EOD') -> Optional[Dict]:
        ...
        if row:
            result = dict(row)
            logger.info(f"[OK] Found cached prediction for {symbol} on {date}")
            return result
    ...
    def get_active_predictions(self) -> List[Dict]:
        ...
        logger.info(f"[OK] Retrieved {len(predictions)} active predictions")
    ...
    def get_prediction_history(self, symbol: str, days: int = 30, 
        ...
        predictions = [dict(row) for row in rows]
        logger.info(f"[OK] Retrieved {len(predictions)} historical predictions for {symbol}")
    ...
    def calculate_accuracy_stats(self, symbol: str, days: int = 30, 
        ...
        logger.info(f"[OK] Calculated accuracy stats for {symbol}: "
    ...
    def update_accuracy_stats(self, symbol: str, timeframe: str = 'DAILY_EOD',
        ...
        logger.info(f"[OK] Updated accuracy stats for {symbol}: {correct}/{total} correct "
    ...
    def get_accuracy_statistics(self, symbol: str, timeframe: str = 'DAILY_EOD',
        ...
        logger.info(f"[OK] Retrieved accuracy statistics for {symbol} ({period})")
