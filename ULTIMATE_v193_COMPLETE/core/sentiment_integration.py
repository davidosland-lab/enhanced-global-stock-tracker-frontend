...
class IntegratedSentimentAnalyzer:
    ...
    def __init__(self, use_finbert: bool = True):
        ...
        if self.use_finbert:
            try:
                # Use local FinBERT model to avoid HuggingFace downloads
    ...
    def load_morning_sentiment(self, market: str = 'au', max_age_hours: int = 24) -> Optional[Dict]:
        ...
        FIX v1.3.15.84: Handle both dated and non-dated morning report files
        """
        try:
            ...
            # FIX v1.3.15.84: Try canonical file first, then dated files
            # FIX v1.3.15.104: Use absolute path from project root
            # Determine project root (go up from core/ directory)
            ...
            if canonical_path.exists():
                ...
                logger.info(f"[SENTIMENT] Found canonical morning report: {canonical_path}")
            else:
                # Fallback: Search for dated files (e.g., au_morning_report_2026-02-03.json)
                import glob
                dated_files = sorted(glob.glob(str(report_dir / f'{market}_morning_report_*.json')), 
                                   reverse=True)  # Newest first
                if dated_files:
                    report_path = Path(dated_files[0])
                    logger.info(f"[SENTIMENT] Using dated morning report: {report_path}")
            
            if not report_path or not report_path.exists():
                logger.warning(f"[SENTIMENT] Morning report not found for market '{market}'")
                logger.warning(f"[SENTIMENT] Searched: {canonical_path} and {market}_morning_report_*.json")
                return None
            ...
            sentiment_data = {
                ...
                # v193: World Event Risk data
                'world_risk_score': 50,
                'world_risk_level': 'MODERATE',
            ...
            # v193: Extract world event risk if available in SPI sentiment
            spi_sentiment = report.get('spi_sentiment', {})
            if 'world_event_risk' in spi_sentiment:
                world_risk = spi_sentiment['world_event_risk']
                sentiment_data['world_risk_score'] = world_risk.get('world_risk_score', 50)
                sentiment_data['world_risk_level'] = world_risk.get('risk_level', 'MODERATE')
                logger.info(f"[SENTIMENT] World Risk: {sentiment_data['world_risk_score']:.0f}/100 "
                           f"({sentiment_data['world_risk_level']})")
    ...
    def _determine_trading_decision(self, market_sentiment: float, market_rec: str,
        ...
        # v193: World event risk gates
        world_risk_score = self.morning_sentiment_cache.get('world_risk_score', 50)
        world_risk_level = self.morning_sentiment_cache.get('world_risk_level', 'MODERATE')
        
        if world_risk_score >= 85:
            # CRITICAL world risk - block new longs if market sentiment is weak
            if market_sentiment < 60:
                decision = 'BLOCK'
                size_multiplier = 0.0
                reasons.append(f"CRITICAL world risk ({world_risk_level}, {world_risk_score:.0f}/100)")
            else:
                # Even with good market sentiment, reduce size on critical risk
                if decision == 'ALLOW':
                    decision = 'REDUCE'
                size_multiplier = min(size_multiplier, 0.5)
                reasons.append(f"CRITICAL world risk - caution ({world_risk_level})")
        
        elif world_risk_score >= 75:
            # ELEVATED world risk - reduce position sizes
            if decision == 'ALLOW':
                decision = 'REDUCE'
            size_multiplier = min(size_multiplier, 0.6)
            reasons.append(f"ELEVATED world risk ({world_risk_level}, {world_risk_score:.0f}/100)")
        
        elif world_risk_score >= 65:
            # HIGH world risk - apply caution
            if decision == 'ALLOW':
                decision = 'CAUTION'
            size_multiplier = min(size_multiplier, 0.75)
            reasons.append(f"HIGH world risk ({world_risk_level})")
        
        elif world_risk_score <= 35:
            # LOW world risk - slight boost if other conditions favorable
            if decision == 'ALLOW' and market_sentiment > 60:
                size_multiplier = min(1.15, size_multiplier * 1.05)  # 5% boost
                reasons.append(f"LOW world risk environment ({world_risk_score:.0f}/100)")
    ...
    def get_trading_gate(self, symbol: str, news_items: List[str] = None, 
                        market: str = 'au') -> Dict:
        """
        Get trading gate decision with full details
        
        This method is called by the dashboard to determine if a trade should be allowed.
        It returns the full sentiment analysis including the decision, reason, and size multiplier.
        
        Args:
            symbol: Stock symbol
            news_items: News headlines for sentiment analysis
            market: Market code ('au', 'us', 'uk')
        ...
        Returns:
            Dict with:
                ...
                - size_multiplier: Position size multiplier (0.0 to 1.2)
                - market_sentiment: Market-wide sentiment (0-100)
                - stock_sentiment: Stock-specific sentiment
        """
        try:
            analysis = self.get_comprehensive_sentiment(symbol, news_items, market)
            ...
            return {
                ...
                'size_multiplier': analysis['position_size_multiplier'],
                'market_sentiment': analysis['market']['sentiment'],
                'market_recommendation': analysis['market']['recommendation'],
                'stock_sentiment_label': analysis['stock']['sentiment_label'],
                'stock_confidence': analysis['stock']['confidence'],
                'stock_compound': analysis['stock']['compound'],
                'timestamp': analysis['timestamp']
            }
        except Exception as e:
            logger.error(f"[SENTIMENT GATE] Error getting trading gate for {symbol}: {e}")
