...
# Add parent directory to access pipelines module
...
try:
    ...
    from pipelines.models.screening.us_overnight_pipeline import USOvernightPipeline as OvernightPipeline
    from pipelines.models.screening.us_stock_scanner import USStockScanner as OriginalScanner
    from pipelines.models.screening.us_market_monitor import USMarketMonitor as SPIMonitor  # Renamed for compatibility
...
class USFullPipelineRunner:
    ...
    def __init__(self, symbols=None, capital=100000, config_path='config/live_trading_config.json',
        ...
        logger.info(f"Initial Capital: USD{capital:,.2f} USD")
    ...
    def run_full_overnight_pipeline(self, mode='full'):
        ...
        try:
            ...
            execution_time = results.get('execution_time_minutes', 'N/A')
            logger.info(f"Execution Time: {execution_time} minutes")
            
            # Handle statistics
            stats = results.get('statistics', {})
            total_stocks = stats.get('total_stocks_scanned', results.get('total_stocks', 0))
            top_count = stats.get('top_opportunities_count', len(results.get('top_opportunities', [])))
            
            logger.info(f"Stocks Scanned: {total_stocks}")
            logger.info(f"Top Opportunities: {top_count}")
            ...
            try:
                ...
                # v193: Load SPI sentiment from trading platform JSON (contains world_event_risk)
                import json
                from pathlib import Path
                
                project_root = Path(__file__).parent.parent
                us_report_path = project_root / 'reports' / 'screening' / 'us_morning_report.json'
                
                spi_sentiment = {}
                if us_report_path.exists():
                    with open(us_report_path, 'r') as f:
                        us_report = json.load(f)
                        spi_sentiment = us_report.get('spi_sentiment', {})
                        logger.info(f"[HTML REPORT] Loaded SPI sentiment from {us_report_path}")
                else:
                    # Fallback to market_sentiment from results
                    spi_sentiment = {
                        'sentiment_score': results.get('market_sentiment', {}).get('score', 50),
                        'gap_prediction': {
                            'predicted_gap_pct': results.get('market_sentiment', {}).get('gap_prediction', 0),
                            'direction': results.get('market_sentiment', {}).get('direction', 'neutral'),
                            'confidence': 50
                        },
                        'recommendation': {
                            'stance': results.get('market_sentiment', {}).get('recommendation', 'NEUTRAL'),
                            'message': 'Hold positions',
                            'risk_level': 'MODERATE'
                        }
                    }
                    logger.warning(f"[HTML REPORT] US report JSON not found, using fallback sentiment")
    ...
    def run(self, check_market_hours=True, mode='full'):
        ...
        if self.use_original_modules:
            ...
            if results:
                ...
                # ================================================================
                # DATA LOGGING: Store predictions and market data for analytics
                # ================================================================
                try:
                    from pipelines.data_storage.pipeline_integration import PipelineDataLogger
                    
                    logger.info("[ANALYTICS] Storing prediction data...")
                    data_logger = PipelineDataLogger(base_path='data/us')
                    
                    # Store top opportunities as predictions
                    if top_opps:
                        pred_stats = data_logger.log_predictions(top_opps)
                        logger.info(f"[ANALYTICS] [OK] Stored {pred_stats['stored']} prediction signals")
                        logger.info(f"[ANALYTICS]   Data path: data/us/trades/")
                    ...
                    if stocks_data:
                        ...
                        if not stocks_df.empty:
                            ...
                            logger.info(f"[ANALYTICS] [OK] Stored market snapshot: {len(stocks_df)} symbols\n")
                    
                except Exception as e:
                    logger.warning(f"[ANALYTICS] Data logging failed (non-critical): {e}\n")
                # ================================================================
            ...
            try:
                ...
                try:
                    ...
                    if predictions:
                        ...
                        logger.info(f"[ANALYTICS] [OK] Stored {pred_stats['stored']} prediction signals")
                        logger.info(f"[ANALYTICS]   Total trades: {pred_stats['total_trades']}")
                        logger.info(f"[ANALYTICS]   Data path: data/us/trades/")
                    
                    # Store market snapshot if available
                    if stocks_data:
                        import pandas as pd
                        if isinstance(stocks_data, list):
                            stocks_df = pd.DataFrame(stocks_data)
                        ...
                        if not stocks_df.empty:
                            market_path = data_logger.log_market_data(stocks_df)
                            logger.info(f"[ANALYTICS] [OK] Stored market snapshot: {len(stocks_df)} symbols")
                    
                    logger.info("[ANALYTICS] Data logging complete\n")
