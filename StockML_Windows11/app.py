#!/usr/bin/env python3
"""
Stock Analysis System - Complete ML Version with Yahoo + Alpha Vantage
Fixed timeframe handling and ML predictions
"""
...
class MLPredictor:
    ...
    def predict(self, df, days=5):
        ...
        for i in range(days):
            ...
            # Limit prediction to realistic range (-5% to +5% per day)
            ...
            last_features_scaled[0][0] = pred_return  # Update returns feature
        
        return predictions

class StockDataFetcher:
    """Enhanced data fetcher with better Yahoo Finance handling"""
    ...
    def fetch_yahoo_data(self, symbol: str, period: str = '1mo') -> tuple:
        ...
        try:
            ...
            if period == '1d':
                # For 1 day, get intraday data
                hist = ticker.history(period='1d', interval='5m', prepost=False)
            elif period == '5d':
                hist = ticker.history(period='5d', interval='30m', prepost=False)
            elif period == '1mo':
                hist = ticker.history(period='1mo', interval='1d', prepost=False)
            elif period == '3mo':
                hist = ticker.history(period='3mo', interval='1d', prepost=False)
            elif period == '6mo':
                hist = ticker.history(period='6mo', interval='1d', prepost=False)
            elif period == '1y':
                hist = ticker.history(period='1y', interval='1d', prepost=False)
            elif period == '5y':
                hist = ticker.history(period='5y', interval='1wk', prepost=False)
            else:
    ...
    def fetch_alpha_vantage_data(self, symbol: str, period: str = '1mo') -> tuple:
        """Fetch from Alpha Vantage with proper time series"""
        try:
            symbol = self._add_au_suffix(symbol).replace('.AX', '')
            print(f"Fetching {symbol} from Alpha Vantage for period {period}")
            ...
            if 'Error Message' in data or 'Note' in data:
                print(f"Alpha Vantage error or limit reached")
                return None, None, None
            
            if time_key not in data:
                print(f"Alpha Vantage: No time series data")
                return None, None, None
    ...
    def fetch_data(self, symbol: str, period: str = '1mo') -> tuple:
        """Fetch data with Yahoo primary and Alpha Vantage fallback"""
        # Always try Yahoo first
        ...
        # Generate test data as last resort
        print(f"Both APIs failed, generating test data for {symbol}")
        return self.generate_test_data(symbol, period), "Test Data", 150.0
    
    def generate_test_data(self, symbol='AAPL', period='1mo'):
        """Generate realistic test data"""
        # Determine number of points based on period
        if period == '1d':
            points = 78  # 5-minute bars for trading day
            freq = '5min'
        elif period == '5d':
            points = 48  # 30-minute bars
            freq = '30min'
        elif period == '1mo':
            points = 30
            freq = 'D'
        ...
        if freq in ['5min', '30min']:
            dates = pd.date_range(end=end, periods=points, freq=freq)
        else:
            dates = pd.date_range(end=end, periods=points, freq=freq)
        
        # Generate realistic prices
        base_price = 150 if 'AAPL' in symbol else 140 if 'CBA' in symbol else 100
        prices = []
        ...
        df = pd.DataFrame({
            ...
            'Close': prices,
            'Volume': [np.random.randint(10000000, 50000000) for _ in range(points)]
        }, index=dates)
...
def get_stock_data(symbol):
    """API endpoint for raw stock data with ML predictions"""
    try:
...
def generate_chart():
    """Generate chart with ML predictions and technical indicators"""
    ...
    try:
        ...
        fig = make_subplots(
            ...
            subplot_titles=['Price & ML Predictions', 'Volume', 'RSI', 'MACD'],
        ...
        if show_ml and len(df) >= 50:
            ...
            if predictions:
                ...
                std_dev = np.std(predictions) * (1 - confidence)
        ...
        # Update layout
        fig.update_layout(
            title=f'{symbol.upper()} - {period.upper()} - {source}',
            height=900,
            showlegend=True,
            hovermode='x unified',
...
@app.route('/')
def index():
    """Main interface with ML predictions"""
    html = '''
<!DOCTYPE html>
<html>
<head>
    <title>Stock Analysis System - ML Edition</title>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .status-test { background: #9C27B0; color: white; }
...
<body>
    <div class="container">
        <div class="header">
            ...
            <p>Professional analysis with Machine Learning predictions, Yahoo Finance + Alpha Vantage</p>
        </div>
    ...
    <script>
        ...
        async function fetchData() {
            ...
            try {
                ...
                alert(`Data fetched successfully!\\n` +
                      ...
                      `Has ML Predictions: ${data.ml_predictions ? 'Yes' : 'No'}`);
            } catch (error) {
        ...
        async function loadChart() {
            ...
            document.getElementById('chart').innerHTML = 
                '<div id="loading"><div class="spinner"></div>Loading chart data...</div>';
            ...
            try {
                ...
                badge.className = 'status-badge ' + 
                    (result.stats.source.includes('Yahoo') ? 'status-yahoo' : 
                     result.stats.source.includes('Alpha') ? 'status-alpha' : 'status-test');
...
if __name__ == '__main__':
    print("=" * 70)
    print("STOCK ANALYSIS SYSTEM - ML COMPLETE EDITION")
    print("=" * 70)
    print("Features:")
    print("✅ Machine Learning Predictions (RandomForest)")
    print("✅ Yahoo Finance Primary + Alpha Vantage Fallback")
    print("✅ All timeframes working (1d, 5d, 1mo, 3mo, 6mo, 1y, 5y)")
    print("✅ Accurate current prices")
