...
class OvernightPipeline:
    ...
    def __init__(self):
        ...
        try:
            ...
            logger.info(f"  LSTM Training Enabled: {self.config.get('lstm_training', {}).get('enabled', False)}")
            logger.info(f"  Max Models Per Night: {self.config.get('lstm_training', {}).get('max_models_per_night', 0)}")
    ...
    def run_full_pipeline(self, sectors: List[str] = None, stocks_per_sector: int = 30) -> Dict:
        ...
        try:
            ...
            results = self._finalize_pipeline(
                ...
                lstm_training_results=lstm_training_results
            )
    ...
    def _assess_event_risks(self, stocks: List[Dict]) -> Dict:
        ...
        logger.info(f"  Market Regime Engine: {'ENABLED' if self.event_guard.regime_available else 'DISABLED'}")
        
        try:
            # Extract tickers
            tickers = [s['symbol'] for s in stocks]
            
            # Batch assess (this will call regime engine once for all stocks)
    ...
    def _finalize_pipeline(self, scored_stocks: List[Dict], spi_sentiment: Dict, report_path: str, lstm_training_results: Dict = None) -> Dict:
        ...
        results = {
            ...
            'statistics': {
                ...
                'lstm_models_trained': lstm_training_results.get('trained_count', 0) if lstm_training_results else 0,
                'lstm_training_failed': lstm_training_results.get('failed_count', 0) if lstm_training_results else 0,
            },
            'market_sentiment': {
