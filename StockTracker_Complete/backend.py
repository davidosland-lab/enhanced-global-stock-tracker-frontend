...
async def analyze_document(request: dict):
    """Analyze document sentiment with FinBERT"""
    text = request.get("text", "")
    symbol = request.get("symbol", "")
    
    # Try to use real FinBERT analyzer
    try:
        import sys
        import os
        # Add parent directory to path to import finbert_analyzer
        sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))
        from finbert_analyzer import analyze_financial_text
        
        # Get real FinBERT analysis
        ...
        # Fallback to deterministic demo mode if FinBERT not available
        # Use text content to generate consistent results
        text_lower = text.lower()
        
        # Count positive and negative words for consistent scoring
        positive_words = ["growth", "profit", "increase", "gain", "strong", "beat"]
        negative_words = ["loss", "decline", "decrease", "fall", "weak", "miss"]
        
        pos_count = sum(1 for word in positive_words if word in text_lower)
        neg_count = sum(1 for word in negative_words if word in text_lower)
        
        # Generate consistent score based on content
...
async def upload_document(files: list = File(None), symbol: str = Form(None)):
...
async def get_market_movers():
    ...
    import random
    # Generate sample market movers
    symbols = ["BHP.AX", "CBA.AX", "CSL.AX", "WBC.AX", "ANZ.AX", "NAB.AX", "WOW.AX", "TLS.AX", "WES.AX", "RIO.AX"]
    ...
    for symbol in symbols:
        ...
        movers.append({
            ...
            "change": change_percent * random.uniform(0.5, 2),
            "changePercent": change_percent,
            "volume": random.randint(1000000, 10000000)
        })
    
    return JSONResponse({
        "gainers": sorted([m for m in movers if m["changePercent"] > 0], key=lambda x: x["changePercent"], reverse=True),
        "losers": sorted([m for m in movers if m["changePercent"] < 0], key=lambda x: x["changePercent"]),
        "active": sorted(movers, key=lambda x: x["volume"], reverse=True),
        "timestamp": datetime.now().isoformat()
    })
