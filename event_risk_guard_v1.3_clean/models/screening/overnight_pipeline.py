...
class OvernightPipeline:
    ...
    def __init__(self):
        ...
        try:
            ...
            # 🆕 Optional: Market Regime Engine (v1.3)
            if MarketRegimeEngine is not None:
                try:
                    self.regime_engine = MarketRegimeEngine()
                    logger.info("✓ Market Regime Engine enabled (HMM/GARCH regime detection)")
    ...
    def run_full_pipeline(self, sectors: List[str] = None, stocks_per_sector: int = 30) -> Dict:
        ...
        try:
            ...
            if self.trainer is not None and self.config.get('lstm_training', {}).get('enabled', True):
                logger.info("\n" + "="*80)
                ...
                try:
                    # Create training queue from scored stocks (prioritized by opportunity score)
                    max_models = self.config.get('lstm_training', {}).get('max_models_per_night', 100)
                    ...
                    if training_queue:
                        ...
                        logger.info(f"  Training time: {training_results.get('total_time', 0)/60:.1f} minutes")
                        logger.info(f"  Success rate: {training_results.get('trained_count', 0)/max(training_results.get('total_stocks', 1), 1)*100:.1f}%")
                    else:
                        logger.info("  No models need training (all models are fresh)")
                        
                except Exception as e:
                    logger.warning(f"⚠ LSTM training failed: {e}")
                    logger.warning(f"  Traceback: {traceback.format_exc()}")
                    # Don't fail pipeline if training fails
    ...
    def _fetch_market_sentiment(self) -> Dict:
        ...
        try:
            ...
            # 🆕 v1.3: Add market regime analysis if available
