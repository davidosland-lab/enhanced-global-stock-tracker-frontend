...
"""
    ...
    <div class="section">
        ...
        {self._build_regime_section(spi_sentiment)}
    </div>
"""
    ...
    def _build_regime_section(self, spi_sentiment: Dict) -> str:
        ...
        if regime_label == 'CALM':
            regime_emoji = "🟢"
            regime_class = "positive"
        elif regime_label == 'HIGH_VOL':
            regime_emoji = "🔴"
            regime_class = "negative"
        else:  # normal
            regime_emoji = "🟡"
        ...
        return f"""
        <div class="regime-info" style="margin-top: 20px; padding: 15px; background: #f8f9fa; border-radius: 8px; border-left: 4px solid #007bff;">
            <h3 style="margin-top: 0; color: #007bff;">🔍 Market Regime Analysis (v1.3)</h3>
            <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 15px;">
                <div>
                    <strong>Regime:</strong> {regime_emoji} <span class="{regime_class}">{regime_label}</span>
                </div>
                <div>
                    <strong>Volatility (1-day):</strong> {vol_1d*100:.2f}%
                </div>
                <div>
                    <strong>Volatility (annual):</strong> {vol_annual*100:.1f}%
                </div>
                <div>
                    <strong>Crash Risk:</strong> {risk_emoji} <span class="{risk_class}">{crash_risk:.2f}/1.0</span>
                </div>
                <div>
                    <strong>Method:</strong> {vol_method.upper()}
                </div>
            </div>
        </div>
"""
