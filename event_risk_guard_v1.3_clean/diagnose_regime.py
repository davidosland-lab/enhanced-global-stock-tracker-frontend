    ...
    for package, note in packages.items():
        try:
            ...
            if note:
                ...
                print(f"[FAIL] {package}: NOT INSTALLED - REQUIRED!")
                return False
...
def check_regime_engine_import():
    ...
    try:
        ...
        print("[OK] MarketRegimeEngine imported successfully")
        return MarketRegimeEngine
    except ImportError as e:
        print(f"[FAIL] Failed to import MarketRegimeEngine: {e}")
        print()
...
def test_yfinance_single_ticker():
    ...
    try:
        ...
        print(f"[OK] Received data: {len(data)} rows")
        if len(data) > 0:
            ...
            print("[FAIL] Received empty dataframe")
            print()
...
def test_yfinance_multiple_tickers():
    ...
    try:
        ...
        if isinstance(data.columns, pd.MultiIndex):
            print(f"  [OK] MultiIndex detected with {data.columns.nlevels} levels")
            ...
            if 'Close' in level_1_values:
                ...
                print(f"  [OK] Extracted Close prices: shape={close.shape}, columns={list(close.columns)}")
                return True, data
            elif 'Adj Close' in level_1_values:
                close = data.xs('Adj Close', level=1, axis=1)
                print(f"  [OK] Extracted Adj Close prices: shape={close.shape}, columns={list(close.columns)}")
                return True, data
            else:
                print(f"  [FAIL] Cannot find 'Close' or 'Adj Close' in level 1")
                print(f"  Available: {list(level_1_values)}")
                return False, None
        else:
            print("  [WARN] Not a MultiIndex (single ticker?)")
            print(f"  Columns: {list(data.columns)}")
            return True, data
...
def test_regime_engine():
    ...
    try:
        ...
        if regime == 'unknown' and method == 'none':
            print("[FAIL] REGIME ENGINE NOT WORKING")
            print()
            ...
            print("[SUCCESS] REGIME ENGINE WORKING!")
            print()
...
def main():
    ...
    if not check_imports():
        print()
        print("[FAIL] Missing required packages. Please run: pip install -r requirements.txt")
        return False
    results.append(True)
    ...
    if engine_class is None:
        print()
        print("[FAIL] Cannot import regime engine modules.")
        return False
    results.append(True)
    ...
    if not test_yfinance_single_ticker():
        print()
        print("[FAIL] Cannot fetch single ticker data. Check internet connection.")
        return False
    results.append(True)
    ...
    if not success:
        print()
        print("[FAIL] Cannot fetch multiple tickers or extract Close prices.")
    ...
    if not test_regime_engine():
        print()
        print("[FAIL] Regime engine test failed.")
        return False
    results.append(True)
