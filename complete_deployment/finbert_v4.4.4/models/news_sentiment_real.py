...
# SESSION-LEVEL RBA CACHE (in-memory, shared across all stocks in a run)
# This prevents re-scraping the same RBA content for every stock
_RBA_CACHE = {
    'data': None,
    'timestamp': None,
    'ttl_minutes': 60  # RBA data valid for 60 minutes
}
...
def scrape_rba_pages(symbol: str) -> List[Dict]:
    ...
    CACHING: Uses session-level cache to avoid re-scraping for every stock
    RBA data is the same for all Australian stocks, so we cache it in memory
    ...
    global _RBA_CACHE
    if _RBA_CACHE['data'] is not None and _RBA_CACHE['timestamp'] is not None:
        age_minutes = (time.time() - _RBA_CACHE['timestamp']) / 60
        if age_minutes < _RBA_CACHE['ttl_minutes']:
            logger.info(f"✓ Using cached RBA data (age: {age_minutes:.1f} min, shared across all stocks)")
            return _RBA_CACHE['data'].copy()
    
    # CACHE MISS - Scrape fresh data
    logger.info(f"Scraping RBA official pages for Australian market context (respectful scraping)...")
    logger.info(f"  (This will be cached and shared across all Australian stocks)")
    articles = []
    ...
    # STORE IN SESSION CACHE for subsequent stocks
    # (global declaration already at top of function)
    _RBA_CACHE['data'] = articles.copy()
    _RBA_CACHE['timestamp'] = time.time()
    logger.info(f"  ✓ RBA data cached (will be reused for all Australian stocks in this run)")
